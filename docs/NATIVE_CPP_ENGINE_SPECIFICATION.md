...
| [Direct2D `ID2D1Geometry::Outline`](https://learn.microsoft.com/en-us/windows/win32/direct2d/id2d1geometry-outline) | Outline removes transverse intersections, normalizes outer contours, and produces fill-mode-invariant geometry in a caller-owned sink. | Emit analytic rectangles directly. For paths, flatten each filled contour with the caller tolerance, normalize orientation, and transactionally accept multiple independent contours only after pairwise boundary and containment rejection. Preserve Direct2D's alternate fill callback and caller segment flags, then compare callback counts and dense regions with Windows ARM64/x64. Reject holes, nesting, touching/overlap, or self-intersection until the common native boolean engine can normalize them exactly; topology walks are scalar by dependency, not an unoptimized data-parallel fallback. |
