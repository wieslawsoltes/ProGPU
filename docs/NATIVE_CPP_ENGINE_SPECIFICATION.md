...
| [Direct2D `ID2D1Geometry::GetWidenedBounds`](https://learn.microsoft.com/en-us/windows/win32/direct2d/id2d1geometry-getwidenedbounds) | Stroke geometry is widened before the optional world transform. | Compute exact nondegenerate rectangle bounds for default and same-factory solid styles by expanding in local space, then transform the four expanded corners. Fail closed for dashed and degenerate cases until shared cap/run bounds are available. |
