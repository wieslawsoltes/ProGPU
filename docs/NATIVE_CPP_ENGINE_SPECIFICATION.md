...
| WPF `CMilGeometryGroupDuce::GetShapeDataCore` and `CDrawingContext::DrawGeometry` in the tracked source tree | A geometry group appends every child figure into one `CShape`; drawing fills first, computes one aggregate stroke bound when the pen brush needs it, then strokes the same multi-figure shape. | Preserve the original child figure/contour boundaries, reset dash state per figure, resolve one group pen brush, compose child/group/drawing transforms in WPF order, and submit fill before stroke. Fail closed for child kinds whose exact stroke contours are not yet represented. |
