...
| [Direct2D `ID2D1Geometry::GetWidenedBounds`](https://learn.microsoft.com/en-us/windows/win32/direct2d/id2d1geometry-getwidenedbounds) | Stroke geometry is widened before the optional world transform; an intrinsic transformed geometry changes the geometry but must not scale the later stroke width. | Keep the analytic rectangle lane. For one simple closed path and the null/default miter stroke, tolerance-flatten locally, derive segment offset endpoints and limited-miter extrema, then affine-transform and reduce four candidates per ARM64 NEON/SSE2 batch. Compare ordinary, concave, zero-width, and nonuniform transformed bounds with system Direct2D. Fail closed for styles, flags, open/multiple figures, degeneracy, or self-intersection. |
