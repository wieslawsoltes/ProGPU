...
| [Direct2D `CreateSharedBitmap`](https://learn.microsoft.com/en-us/windows/win32/api/d2d1/nf-d2d1-id2d1rendertarget-createsharedbitmap) | A resource-compatible `ID2D1Bitmap` view shares the original data and may select independent DPI or alpha interpretation; the DXGI format must match. WIC-lock and DXGI-surface inputs have separate ownership constraints. | First implement exact same-factory premultiplied bitmap views: retain the original resource, forward mutation, keep one live generation/storage identity, and deduplicate retained/GPU image upload across views. Fail closed for unsupported alpha reinterpretation, compatible-scene bitmaps, WIC locks, DXGI surfaces, or foreign domains until their typed ownership contracts exist. |
