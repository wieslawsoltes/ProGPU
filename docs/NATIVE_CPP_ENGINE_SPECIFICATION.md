...
| [Direct2D `ID2D1Geometry::StrokeContainsPoint`](https://learn.microsoft.com/en-us/windows/win32/direct2d/id2d1geometry-strokecontainspoint) | The centered stroke contains its outer and inner boundaries; geometry is stroked before the optional world transform. | Keep the analytic rectangle lane. For one simple closed path with a null/default miter or same-factory solid bevel/miter/miter-or-bevel/round style, inverse-map the point through an arbitrary nonsingular affine world transform, evaluate four point-to-segment distances per NEON/SSE2 batch, then test the selected topology-dependent join wedge, typed miter limit, or exact round vertex disk. Compare straight edges, convex/concave joins, bevel and round clipping, interior/exterior points, and transformed probes with system Direct2D. Fail closed for dashed styles, segment flags, open/multiple figures, degeneracy, self-intersection, and singular transforms. |
