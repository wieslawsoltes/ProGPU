...
| [Direct2D `ID2D1Geometry::CombineWithGeometry`](https://learn.microsoft.com/en-us/windows/win32/direct2d/id2d1geometry-combinewithgeometry) | Union, intersection, xor, and exclusion write normalized filled geometry to the caller sink after transforming only the input operand. Boundary topology remains observable by later fill, outline, and stroke operations. | Keep the exact fixed-grid tracer for axis-aligned rectangle operands. For independently affine-transformed rectangles, split both convex boundaries at bounded pairwise and coincident-edge endpoints, classify Boolean membership on both sides of every sub-edge, deduplicate coincident directed boundaries, and trace allocation-free closed contours through the caller sink. Compare all four modes, both transformed-operand positions, identical rectangles, full/partial shared edges, and collinear-overlap regions with system Direct2D; fail closed before sink mutation for degeneracy or non-rectangle inputs. |
