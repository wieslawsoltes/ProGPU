...
| [Direct2D `ID2D1Geometry::GetWidenedBounds`](https://learn.microsoft.com/en-us/windows/win32/direct2d/id2d1geometry-getwidenedbounds) | Stroke geometry is widened before the optional world transform; an intrinsic transformed geometry changes the geometry but must not scale the later stroke width. | Compute exact nondegenerate rectangle bounds for default and same-factory solid styles by expanding in local space, then transform the four expanded corners. For axis-preserving transformed rectangles, materialize the intrinsic rectangle first, widen with the unscaled stroke, and only then apply the caller transform. Fail closed for dashed, degenerate, and general affine transformed cases until shared cap/run/offset bounds are available. |
| [Direct2D `ID2D1Geometry::StrokeContainsPoint`](https://learn.microsoft.com/en-us/windows/win32/direct2d/id2d1geometry-strokecontainspoint) | The centered default stroke contains its outer and inner boundaries; an intrinsic transformed geometry changes the path before the unscaled stroke is applied. | For nondegenerate rectangles, test the transformed expanded miter rectangle and subtract only the strict transformed inner rectangle. Support axis-preserving intrinsic transformed rectangles and arbitrary nonsingular caller transforms; fail closed for explicit styles, degeneracy, and singular transforms until shared styled-offset containment lands. |
| [Direct2D `ID2D1Geometry::Widen`](https://learn.microsoft.com/en-us/windows/win32/direct2d/id2d1geometry-widen) | Widen writes caller-owned filled contours for the centered stroke after intrinsic geometry transforms and before the optional caller transform. Sink fill mode, segment flags, figure closure, and bridge points are observable compatibility state. | Emit Direct2D's alternate-fill closed outer/inner miter contours for base rectangles. For positive axis-aligned intrinsic transforms with a surviving inner contour, emit the system-compatible single winding-fill, force-unstroked open bridge contour, then apply the caller transform. Fail closed for zero width, styles, degeneracy, transformed collapsed-inner cases, and other intrinsic transforms until their exact system transcripts are qualified. |
| [Direct2D `ID2D1Geometry::CompareWithGeometry`](https://learn.microsoft.com/en-us/windows/win32/direct2d/id2d1geometry-comparewithgeometry) | The relation describes this geometry relative to the optionally transformed input geometry; `IS_CONTAINED` means this geometry is inside the input. Boundary contact is an overlap, and equal rectangles report `IS_CONTAINED`. | Resolve same-factory rectangle and bounded transformed-rectangle inputs through typed COM interfaces, compose intrinsic transforms before the caller input transform, and compare exact axis-aligned bounds without allocation. Preserve `UNKNOWN` on rejection and fail closed for degeneracy and general affine inputs until the shared topology engine is available. |
| [Direct2D `ID2D1Geometry::CombineWithGeometry`](https://learn.microsoft.com/en-us/windows/win32/direct2d/id2d1geometry-combinewithgeometry) | Union, intersection, xor, and exclusion write normalized filled geometry to the caller sink after transforming only the input operand. Boundary topology remains observable by later fill, outline, and stroke operations. | For axis-aligned rectangle and transformed-rectangle operands, classify a fixed coordinate grid, label four-connected components, trace only exterior/hole edges, coalesce collinear vertices, and emit Direct2D-compatible alternate-fill, force-unstroked closed contours with explicit closing points. Keep the implementation allocation-free and compare all boundary edges with system Direct2D; fail closed before sink mutation outside the qualified domain. |
...
640x360 scene on hardware and Parallels, while Microsoft Basic Render Driver
executes the identical logical scene at 320x180 and 0.5 DPI. It still compiles
and submits every command, verifies stable retained resources across a second
frame, reads the GPU target, and applies DPI-scaled pixel probes. This bounded
physical-resolution concession avoids the software adapter's deterministic
full-frame device loss without weakening the hardware contract or adding a CPU
rendering fallback. The zero-radius edge-aliased point is structurally checked
in the reduced lane and pixel-checked at full resolution because it is
subpixel-degenerate at 0.5 DPI.
