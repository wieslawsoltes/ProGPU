...
| [Direct2D `ID2D1Geometry::CombineWithGeometry`](https://learn.microsoft.com/en-us/windows/win32/direct2d/id2d1geometry-combinewithgeometry) | Union, intersection, xor, and exclusion write normalized filled geometry to the caller sink after transforming only the input operand. Boundary topology remains observable by later fill, outline, and stroke operations. | Keep the exact fixed-grid tracer for axis-aligned rectangle operands. For independently affine-transformed rectangles, split both convex boundaries at bounded pairwise intersections, classify each sub-edge against the other operand, orient selected Boolean boundaries with the filled side consistently, and trace allocation-free closed contours through the caller sink. Compare all four modes and both transformed-operand positions with system Direct2D; fail closed before sink mutation for degeneracy, overlapping collinear edges, or non-rectangle inputs. |
