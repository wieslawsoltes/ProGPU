...
- Linux kernel
  ...
  gain/stereo-balance nodes in declaration order. A fixed 1,024-frame PCM16
  kernel accumulates Q15-scaled sources into signed 64-bit lanes and saturates
  once after all contributors. The existing bounded `sowt`/`twos` reader can
  now materialize pooled PCM16 directly from signed 16-, 24-, or 32-bit
  samples with no steady-read allocation. A monotonic ISO-BMFF source adapter
  retains the sample index and one converted media sample, crosses sample
  boundaries without concatenation, fills declared timestamp gaps with
  silence, and streams blocks directly to a future native encoder callback.
  Planning is O(C + B + O + E); mixing is O(S) for S scalar samples and uses
  O(1,024 × channels) pooled accumulator/output storage plus bounded per-source
  demux metadata, with no duration-sized PCM store.
  PipeWire's official
  ...
  an AAC transform. The WinUI `CreateMp4` contract is H.264 plus AAC, so the
  Linux precise provider continues to reject transformed-audio requests until
  a real AAC codec boundary is runtime-proven. It does not write `sowt` PCM
  under an AAC profile or claim the new mixer is already connected to the
  final MP4. Compatible identity AAC still uses the compressed edit-list lane.
