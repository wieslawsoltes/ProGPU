...
Color-only thumbnail timelines use the same captured order, timing, placement,
opacity, and retained WebGPU compositor before the explicit aligned
readback/PNG boundary. A URI-main thumbnail plus overlays is still rejected:
the current two-candidate decoder cache cannot safely materialize multiple
composition-time variants without another retained base-image lease. URI
overlays and custom compositor definitions are also rejected until a bounded,
explicitly synchronized retained decoder-owner contract exists.

Focused gates cover declared order, exact half-open timing, off-canvas
placement, opacity, GPU capability selection, URI/custom rejection,
color-thumbnail selection, zero allocation across 100,000 timeline checks,
retained compositor/source reuse, RGBA-to-NV12 processing, and the absence of
`Marshal.Copy`, mapped export memory, or device-wide idle waits. These are
contract, build, and managed-allocation results. No Linux media device was
attached, so GBM/Dawn/V4L2 interoperability, rendered pixels, fence behavior,
encoder output, throughput, residency, power, and thermal behavior remain
unproven hardware gates.
