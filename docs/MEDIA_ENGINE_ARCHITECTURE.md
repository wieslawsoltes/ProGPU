...
| Browser | HTML media element now; dependency-free ISO-BMFF fast export plus WebGPU/MediaRecorder effect-bake export; WebCodecs provider remains extensible | `GPUExternalTexture`/browser external image copied into WebGPU texture; export uses OffscreenCanvas plus explicit `ImageBitmap` ownership transfer to the capture canvas | Native Web Audio source, typed GainNodes, StereoPanner, and export-time clip/background/overlay volumes multiplied by registered serialized gain effects; AudioWorklet extension planned | Browser-controlled copy when direct external import is unavailable; CORS security can silence media-element graph sources; effect-baked export is real-time, user-initiated, and codec availability is runtime-probed |
