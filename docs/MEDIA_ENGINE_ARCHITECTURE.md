...
- [`IMFMediaEngineEx::SetBalance`](https://learn.microsoft.com/en-us/windows/win32/api/mfmediaengine/nf-mfmediaengine-imfmediaengineex-setbalance)
  ...
  lifetime. Left and right Q15 levels are computed once per source. Saturation
  ...
  no managed PCM array or duration-sized PCM store exists.

  Schedule capture is O(C + B + O) time and storage for main clips,
  background tracks, and overlays. Rendering is O(F × L) for F output frames
  and L active layers; managed working storage is O(A) for A scheduled
  ...
  `Gain` and `StereoBalance` nodes before native export begins. Unregistered,
  unsupported, non-finite, and greater-than-2× graphs fail capability
  selection. Arbitrary MFT/WASAPI processing is still rejected rather than
  being silently omitted, and Windows codec/runtime, audible-output, latency,
  and allocation evidence remains a hardware gate.
- Apple
...
### Linux solid-color overlay checkpoint

The Linux precise exporter now accepts standard positioned, delayed, and
opacity-controlled solid-color overlays. The clean-room contract comes from
the official
...
Capture flattens O overlays once into immutable half-open intervals, normalized
placements, and typed effect plans in O(O) time and storage. Each output frame
checks O overlays without managed allocation and submits one retained
source-over pass for every active overlay, with O(P) fragment work for P
covered output pixels. The encoder retains one reusable straight-alpha RGBA
color source and one `GpuTextureLayerCompositor`; these are constant-size
resources and do not grow with duration or overlay count. A Gaussian applied
to an unbounded constant-color source is unchanged, so only its affine color
transform is evaluated. URI main frames with active overlays remain entirely
on the GPU through NV12-to-RGBA, retained layer composition, and RGBA-to-NV12
...
Color and URI main-track thumbnail timelines use the same captured order,
timing, placement, opacity, and retained WebGPU compositor before the explicit
aligned readback/PNG boundary. The nearest-frame scheduler can select the same
decoded main frame for multiple composition times with different overlay
states, so a final encoded thumbnail is not cached in that case. Instead, the
renderer lazily retains exactly two output-sized RGBA GPU snapshots matching
the scheduler's existing previous/current candidates. Each decoded lease is
imported and effect-processed once into its slot; every selected composition
time performs a GPU texture copy into the final target, applies its active
overlays, reads back once, and returns the slot when the candidate leaves the
window. This adds O(2P) bounded GPU storage for P output pixels only when a URI
thumbnail timeline has overlays, never grows with the request count or
duration, and performs no CPU re-upload. URI overlays and custom compositor
definitions remain rejected until a bounded, explicitly synchronized retained
overlay-decoder owner exists.

Focused gates cover declared order, exact half-open timing, off-canvas
placement, opacity, GPU capability selection, URI-overlay/custom rejection,
color/URI-main thumbnail selection, the exact two-candidate snapshot bound,
composition-time preservation through key-frame normalization, zero allocation
across 100,000 timeline checks, retained compositor/source reuse,
RGBA-to-NV12 processing, and the absence of `Marshal.Copy`, mapped export
memory, or device-wide idle waits. These are contract, build, and
managed-allocation results. No Linux media device was attached, so
