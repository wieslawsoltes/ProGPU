...
composition timestamp it maps delay and trim to source time, retains the
latest decoded candidate at or before that time plus the first look-ahead
candidate, and imports only the selected NV12 DMA-BUF planes. The selected
image is effect-processed into one reusable source-sized RGBA GPU texture; a
Gaussian URI overlay lazily adds exactly two source-sized blur textures.
...
duration, and performs no CPU re-upload. URI overlays in thumbnail batches and
custom compositor definitions remain rejected; precise export now owns the
bounded URI-overlay decoder contract described above.
...
color/URI-main thumbnail selection, explicit URI-overlay thumbnail rejection,
the exact two-candidate thumbnail snapshot bound, composition-time
preservation through key-frame normalization, zero allocation across 100,000
timeline checks, retained compositor/source reuse, RGBA-to-NV12 processing,
and the absence of `Marshal.Copy`, mapped export memory, or device-wide idle
waits. These are contract, build, and managed-allocation results. No Linux
media device was attached, so
