...
- `BrowserWebGpuMediaCompositionExportProvider` is selected only when a
  ...
  gains/background tracks. A blurred URI visual retains two RGBA8 work
  textures and two 832-byte uniform buffers; horizontal and vertical passes
...
| Android | Implemented MediaPlayer/MediaCodec playback, MediaExtractor/MediaCodec/MediaMuxer precise export, and MediaMetadataRetriever composition thumbnails; export surfaces and video-only colors bypass CPU pixel access, while thumbnail URI decode follows Android's native Bitmap-returning contract | Playback uses ImageReader/AHardwareBuffer plus same-device Dawn Vulkan import/presentation; export uses a SurfaceTexture or WebGPU-generated color through the bounded encoder-Surface GPU path. Registered Gaussian URI effects reuse the shared separable WebGPU kernel and one retained intermediate before the encoder target; thumbnail effects reuse that renderer before one CPU-readable ImageReader/PNG boundary | Registered playback gain and stereo balance fold into left/right `MediaPlayer` volume plus a common per-session `LoudnessEnhancer`; export preserves exactly matching AAC access units on URI-only timelines, while general AAudio processing, stereo-balance export, silence generation, mixing, and gain baking remain planned | Missing exact-ABI Dawn packaging selects an explicitly diagnosed affine playback/export fallback, but Gaussian composition fails capability selection without Vulkan Dawn. Encoded thumbnails are not zero-copy and still need Android device validation; thumbnail overlays and unsupported export audio/composition fail explicitly; hardware encode remains runtime-negotiated |
...
only graph nodes that its selected provider can faithfully bake; the current
shared resolver rejects `StereoBalance` rather than dropping or approximating
it as gain.
...
The existing export resolver deliberately rejects this non-gain kind.

At this checkpoint the complete Release core suite passed 2,684 tests and the
Release headless suite passed 215 tests. The focused media/provider/sample
selection passed 114 tests, JavaScript syntax validation passed, and Release
builds completed with zero warnings/errors for shared media, Windows media,
browser, Android media, Apple media for macOS and iOS, and the macOS ARM64
`ProGPU.Samples.Desktop` app. The rebuilt Desktop app loaded and played the
...
effect completed without a crash. No matched latency, audio-glitch, power, or
Windows/Android/browser device measurement is claimed by this checkpoint.
