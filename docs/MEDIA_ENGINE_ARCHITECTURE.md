...
- [`IMFMediaEngineEx::SetBalance`](https://learn.microsoft.com/en-us/windows/win32/api/mfmediaengine/nf-mfmediaengine-imfmediaengineex-setbalance)
  ...
  effect. Effects remain block-local: latency, look-ahead, and tails are not
  modeled. Unregistered, unsupported, non-finite, and greater-than-2× folded
  graphs fail capability selection. Arbitrary MFT/WASAPI live processing is
  still unclaimed, and Windows codec/runtime, audible-output, latency, and
  allocation evidence remains a hardware gate.
- Apple
...
created exactly one `AudioWorkletNode`, completed play/pause/seek/replay with
result `0`, and returned its DOM media-element count to zero. A fresh
user-activated nonlinear-editor effect/audio export created exactly one
worklet node in the ordered clip graph and completed with result `0`. Both
runs reported WebGPU and AudioWorklet availability and emitted zero console
warnings or errors. This is functional, graph-construction, and lifetime
evidence; it is not a latency, throughput, power, zero-copy, audible-output,
or output-quality measurement.
...
present. Effects cannot yet declare latency, look-ahead, or tails.
