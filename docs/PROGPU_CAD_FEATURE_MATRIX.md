...
| Editing | typed atomic commands, undo/redo, dirty generations | in progress | bounded generation-synchronized history plus translate/origin-axis rotate/pivoted uniform scale; entity duplicate, visibility, layer, linetype, linetype-scale, lineweight, color, and transparency commands; layer add, visibility, plot eligibility, color, lineweight, and linetype commands; single model-space add/remove; caller-buffered generation-tagged BVH candidates; and exact allocation-free point hits for lines, conformal circles/arcs, conformal signed-bulge and straight 2D polylines, 3D polylines, filled SOLIDs, and visible 3DFACE edges, with explicit unsupported results for remaining kinds; remaining exact geometry/window selection, populated-layer removal, pivoted rotation, broader property/tool families, and fuzz tests remain |
