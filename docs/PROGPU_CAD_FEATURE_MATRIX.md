...
| Text | complete ProGPU Unicode/OpenType text stack | in progress | single-line TrueType TEXT now uses typed font resolution, retained ProGPU shaping/fallback runs, conservative affine bounds, and vector glyph-run recording; add aligned/Fit, MTEXT formatting/columns/background, vertical styles, CAD font search/substitution UI, and broader bidi/color/variable-font regressions |
