...
| Editing | typed atomic commands, undo/redo, dirty generations | in progress | bounded generation-synchronized history plus translate/visibility/layer/lineweight assignment and single model-space add/remove commands, handle-reassignment-safe identity retention, cancellation, divergence, and branch tests implemented; broader multi-entity/property/tool command families and fuzz tests remain |
