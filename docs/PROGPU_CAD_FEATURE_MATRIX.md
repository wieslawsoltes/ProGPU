...
| Text | complete ProGPU Unicode/OpenType text stack | in progress | single-line TrueType TEXT now uses typed font resolution, retained ProGPU shaping/fallback runs, two-point Align/Fit scaling, conservative affine bounds, and vector glyph-run recording; add MTEXT formatting/columns/background, decorations, vertical styles, CAD font search/substitution UI, and broader bidi/color/variable-font regressions |
