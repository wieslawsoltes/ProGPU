...
| Editing | typed atomic commands, undo/redo, dirty generations | in progress | bounded generation-synchronized history plus translate/origin-axis rotate/pivoted uniform scale; entity duplicate, visibility, layer, linetype, linetype-scale, lineweight, color, and transparency commands; layer add, visibility, plot eligibility, color, lineweight, and linetype commands; single model-space add/remove; and caller-buffered generation-tagged BVH selection candidates, with handle-reassignment-safe identity retention, table preflight, cancellation, divergence, and branch tests; exact geometry selection, populated-layer removal, pivoted rotation, broader property/tool families, and fuzz tests remain |
