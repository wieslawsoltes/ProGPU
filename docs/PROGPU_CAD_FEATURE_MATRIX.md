...
| Editing | typed atomic commands, undo/redo, dirty generations | in progress | bounded generation-synchronized history plus translate/pivoted axis-angle rotate/pivoted uniform scale; entity duplicate, visibility, layer, linetype, linetype-scale, lineweight, color, and transparency commands; layer add, visibility, plot eligibility, color, lineweight, and linetype commands; single model-space add/remove; caller-buffered generation-tagged BVH candidates and deterministic semantic root-handle deduplication; exact allocation-free point hits for lines, conformal circles/arcs, conformal signed-bulge and straight 2D polylines, 3D polylines, filled SOLIDs, and visible 3DFACE edges; and exact world-space box Window/Crossing tests for lines, affine circles/arcs/ellipses and bulges, straight 2D/3D polylines, filled SOLIDs, and visible 3DFACE edges, with explicit unsupported results for remaining kinds; spline/text and screen-projected selection, draw-order resolution, populated-layer removal, broader property/tool families, and fuzz tests remain |
