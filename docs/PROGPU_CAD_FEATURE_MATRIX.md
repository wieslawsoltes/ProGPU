...
| Editing | typed atomic commands, undo/redo, dirty generations | in progress | bounded generation-synchronized history plus translate/origin-axis rotate/visibility/layer/lineweight/color/transparency assignment and single model-space add/remove commands, handle-reassignment-safe identity retention, cancellation, divergence, and branch tests implemented; pivoted transforms, broader multi-entity/property/tool command families, and fuzz tests remain |
