...
| Editing | typed atomic commands, undo/redo, dirty generations | in progress | bounded generation-synchronized history plus translate/origin-axis rotate/pivoted uniform scale; entity visibility, layer, linetype, linetype-scale, lineweight, color, and transparency assignment; layer visibility, plot eligibility, color, lineweight, and linetype assignment; and single model-space add/remove commands, with handle-reassignment-safe identity retention, table preflight, cancellation, divergence, and branch tests; pivoted rotation, broader property/tool families, and fuzz tests remain |
