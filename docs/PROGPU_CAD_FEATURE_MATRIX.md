...
| Editing | typed atomic commands, undo/redo, dirty generations | in progress | bounded generation-synchronized history plus translate/origin-axis rotate/pivoted uniform scale/entity visibility/layer assignment/layer visibility/linetype and linetype-scale/lineweight/color/transparency assignment and single model-space add/remove commands, handle-reassignment-safe identity retention, table preflight, cancellation, divergence, and branch tests implemented; plot-state commands, pivoted rotation, broader property/tool families, and fuzz tests remain |
