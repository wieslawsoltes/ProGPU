...
`GraphicsStringFormatBenchmarks.MeasureSpan` guards one warmed `ReadOnlySpan<char>` measurement through the same typed shaping, wrapping, bidi, fallback, and cluster layout used for retained drawing and character-range geometry. The 2026-08-22 ARM64/.NET 10.0.11 in-process ShortRun measured a 10.709 µs median (11.316 µs mean, 1.490 µs standard deviation) with 6,712 B/op. One launch, three warmups, three measured iterations, and denied process-priority elevation make this coarse managed-layout evidence. The focused suite independently enforces a 16,384-byte upper allocation bound and covers span/string equality, typed glyph recording, wrapped selection regions, clipped versus `NoClip` bounds, and official empty-input validation order. Contract, architecture, remaining semantics, and gate evidence are recorded in [`docs/research/system-drawing-string-format-contract.md`](research/system-drawing-string-format-contract.md).
