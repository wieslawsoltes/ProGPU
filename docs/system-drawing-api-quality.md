...
The point/source-rectangle image-overload slice reuses the existing texture retention, unit conversion, sampling, remap, color-matrix, and callback path. Five focused tests cover exact point and integer placement, unscaled size preservation, clipping without stretching, point-anchored source cropping, remap attributes, abort callbacks, and null validation. Contract and remaining affine/perspective work are recorded in [`docs/research/system-drawing-graphics-image-overloads-contract.md`](research/system-drawing-graphics-image-overloads-contract.md).
