...
System.Drawing suite passes 620/620 in Debug and Release; ApiCompat remains at
...
The official dotnet/winforms behavior suite is also source-built unchanged
against ProGPU's `System.Drawing.Common` identity:

```bash
./eng/progpu-verify-upstream-system-drawing-tests.sh
```

Its pinned Linux ARM64 baseline enables 82 of 89 source files and records 3,263
passes, 1,105 known failures, and 85 skips across 4,453 expanded cases. Exact
method/exception counts, skips, corpus shape, source exclusions, and test-method
exclusions are reviewed manifests; both regressions and unreviewed improvements
fail. Native HDC, printer-handle, enhanced-metafile, and HICON boundaries remain
explicit debt rather than fake portable objects. Full provenance and the wider
corpus roadmap are recorded in
[`docs/research/system-drawing-corpus-gates.md`](research/system-drawing-corpus-gates.md).
The corresponding reviewed hosted Linux x64 inventory is 3,279 passes, 1,140
known failures, and 34 skips over the same case set. All 220 official Matrix
cases now pass; the same tranche also resolves dependent graphics, region,
gradient, and texture contracts without adding a platform handle or renderer
fork.
...
authoritative. The complete System.Drawing suite passes 620/620 in both Debug
