...
ProGPU stores the six affine elements in `System.Numerics.Matrix3x2`. Parallelogram construction solves the two source axes independently. Composition, rotation about a pivot, shear, inversion, point/vector transforms, cloning, value equality, disposal, and validation are implemented without a GPU device or native graphics handle. The managed layer preserves observable GDI+ behavior for disposed-object `ArgumentException` boundaries, parameterless invalid-order errors, empty point batches, non-finite invertibility, self-multiplication rejection, object-identity hashing, its narrow identity tolerance, and saturation when finite affine arithmetic exceeds `Single` range. Explicit NaN and infinity inputs retain IEEE propagation. `RotateAt` also retains the native identity-query quirk independently of its exported elements; `Reset` and direct element replacement clear that state.
...
The pinned official dotnet/winforms corpus is the behavior authority for this
tranche. All 220 expanded `MatrixTests` cases now pass on Linux ARM64, up from
162, and the complete 4,453-case run removes 72 known failures after dependent
graphics, region, gradient-brush, and texture-brush observations are included.
The local Release `System.Drawing.Common.Tests` suite passes 621/621. Exact
ARM64 and hosted-x64 manifests remain strict CI gates, so a changed exception,
stale known failure, or architecture-specific difference cannot be silently
accepted.
