...
Ordinary point, rectangle, formatted, and explicit-character-advance `Graphics.DrawString` paths retain `FontStyle.Underline` and `FontStyle.Strikeout` as typed rectangle commands alongside the retained text or shaped-glyph commands. Each visual line uses the selected face's OpenType [`post.underlinePosition` and `post.underlineThickness`](https://learn.microsoft.com/en-us/typography/opentype/spec/post) or [`OS/2.yStrikeoutPosition` and `OS/2.yStrikeoutSize`](https://learn.microsoft.com/en-us/typography/opentype/spec/os2), scaled from design units to the requested pixel size; bounded metric fallbacks are used only when an optional table value is absent. This matches the public [`FontStyle`](https://learn.microsoft.com/en-us/dotnet/api/system.drawing.fontstyle) contract without treating decoration bits as typeface-selection coordinates. Brushes and graphics transforms are preserved, and rectangle-layout decorations remain inside the same retained clip as their glyphs.
