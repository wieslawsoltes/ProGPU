...
The known-color cache preserves the framework's deliberately asymmetric contract. `Brushes.*` publishes process-wide mutable `SolidBrush` instances, while `SystemBrushes.*`, `Pens.*`, and `SystemPens.*` remain immutable and reject mutation with `ArgumentException`. Cloning any cached resource produces an ordinary mutable, independently disposable object. The official `BrushesTests` corpus mutates and restores every named brush and is the behavioral authority for this otherwise surprising distinction.
