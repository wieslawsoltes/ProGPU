...
`DisplayFormatControl` preserves default ignorables through the typed shaping buffer. A control without a nominal cmap entry records the font's outlined `.notdef` glyph when that glyph has geometry. Fonts with an empty glyph zero instead select the first outlined square, dotted-circle, replacement, or question-mark representative available on the same face, avoiding the invisible space substitution used by the normal path while keeping one typed text renderer. `HotkeyPrefix.Show` removes the first unescaped ampersand, retains the shaped UTF-16 cluster index of the following scalar, and records one filled underline rectangle using the owning shaped face's underline metrics. `HotkeyPrefix.Hide` performs the same ampersand unescaping without decoration; doubled ampersands remain one literal ampersand. The decoration shares the glyph run's brush, origin, transform, and active layout clip.
