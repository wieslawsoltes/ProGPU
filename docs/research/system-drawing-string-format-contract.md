...
The next typed semantics slices implement explicit tab-stop expansion, culture-native digit substitution, vertical and RTL base directions, `NoFontFallback`, `MeasureTrailingSpaces`, visible `DisplayFormatControl` representatives, and retained mnemonic underlines for `HotkeyPrefix.Show`. Exact `LineLimit` and ellipsis-path behavior remain explicit follow-up semantics.
...
- bounded warmed allocation for mnemonic recording; and
