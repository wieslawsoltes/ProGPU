...
This slice restores all 23 .NET 10.0.11 types in `System.Drawing.Imaging.Effects`, the official `Bitmap.ApplyEffect(Effect, Rectangle)` member, and both `Graphics.DrawImage` effect overloads. Public type names, inheritance, constructors, properties, enum identities, validation ranges, matrix constants, lookup-table layout, ownership, and XML contract descriptions are based on the upstream `dotnet/winforms` managed source already vendored by LibreWinForms and the pinned `Microsoft.WindowsDesktop.App.Ref` reference assembly.
...
Effect drawing snapshots the bitmap, applies the effect without mutating the caller's image, and retains the effected texture through the existing typed resource lifetime. The long overload converts the source rectangle to pixels, validates it before recording, maps its four corners through the caller's affine `Drawing2D.Matrix`, and then composes the ordinary `Graphics` world/page/host transform. `ImageAttributes` run after the effect through the existing exact managed adjustment path. Non-bitmap image storage fails at an explicit boundary.

## Gates and measured debt

Twenty focused cases cover the canonical white-to-sepia pixel, clipped rectangles, straight/premultiplied alpha handling, lookup-table ownership and padding, color-matrix snapshot behavior, deterministic blur pixels, zero-amount sharpening, disposed use, official validation ranges, exactly zero managed bytes across 128 warmed pointwise applications, effect-draw pixels and source ownership, crop/transform/attribute composition, and validation before command recording. The 2026-08-27 ARM64/.NET 10.0.11 ShortRun measured 256×256 pointwise inversion at a 756.184 µs median, radius-eight blur at a 1.744 ms median, and retained 64×64 inversion draw at a 282.715 µs median with 17,200 B allocated. Pointwise and blur operations allocated zero managed bytes. One launch and three measured iterations make this coarse local evidence rather than a renderer-wide claim.

Strict ApiCompat removes 23 missing-type suppressions plus the `Bitmap.ApplyEffect` and two `Graphics.DrawImage` member suppressions, with no new breaking or stale diagnostics. Measured debt moves from 40 missing types, 127 missing members, 17 other diagnostics, and 184 total to 17 missing types, 124 missing members, 17 other diagnostics, and 158 total.
