...
| --- | --- | --- | --- |
| 12.0.5 | `ProGPU.Avalonia.Rendering` `12.0.5-preview.33` | `Avalonia.ProGpu` | ProGPU/WebGPU rendering backend |
| 12.0.5 | `ProGPU.Avalonia.SilkNet` `12.0.5-preview.33` | `Avalonia.SilkNet` | Cross-platform Silk.NET windowing backend |
| 11.3.18 | `ProGPU.Avalonia.Rendering` `11.3.18-preview.33` | `Avalonia.ProGpu` | Shared-source ProGPU/WebGPU rendering backend |
| 11.3.18 | `ProGPU.Avalonia.SilkNet` `11.3.18-preview.33` | `Avalonia.SilkNet` | Shared-source Silk.NET windowing backend |
| native ABI | `ProGPU.Backend.Dawn` `0.1.0-preview.33` | `ProGPU.Backend.Dawn` | Exact-ABI Dawn device, IOSurface shared memory, and timeline fences |
...
use ProGPU `0.1.0-preview.33`. They intentionally use `ProGPU.*` package IDs;
...
Pack the ProGPU `0.1.0-preview.33` portable runtime packages first, then pack
...
1. Tag and publish ProGPU `0.1.0-preview.33`.
2. Confirm the required ProGPU packages are available from NuGet.org.
3. Pack and test both Avalonia integration lanes.
4. Publish the two `12.0.5-preview.33` packages and the two
   `11.3.18-preview.33` packages.
...
<ItemGroup>
  ...
  <PackageReference Include="ProGPU.Avalonia.Rendering" Version="12.0.5-preview.33" />
  <PackageReference Include="ProGPU.Avalonia.SilkNet" Version="12.0.5-preview.33" />
</ItemGroup>
```
...
`11.3.18-preview.33` and pin the Avalonia packages to `11.3.18`.
