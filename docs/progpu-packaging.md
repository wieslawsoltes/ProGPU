...
| --- | --- | --- | --- |
| 12.0.5 | `ProGPU.Avalonia.Rendering` `12.0.5-preview.54` | `Avalonia.ProGpu` | ProGPU/WebGPU rendering backend |
...
use ProGPU `0.1.0-preview.54`. They intentionally use `ProGPU.*` package IDs;
...
Pack the ProGPU `0.1.0-preview.54` portable runtime packages first, then pack
...
1. Tag and publish ProGPU `0.1.0-preview.54`.
2. Confirm the required ProGPU packages are available from NuGet.org.
3. Pack and test both Avalonia integration lanes.
4. Publish the two `12.0.5-preview.54` packages and the two
   `11.3.18-preview.54` packages.
...
<ItemGroup>
  ...
  <PackageReference Include="ProGPU.Avalonia.Rendering" Version="12.0.5-preview.54" />
  <PackageReference Include="ProGPU.Avalonia.SilkNet" Version="12.0.5-preview.54" />
</ItemGroup>
```
...
`11.3.18-preview.54` and pin the Avalonia packages to `11.3.18`.
