...
| --- | --- | --- | --- |
| 12.1.1 | `ProGPU.Avalonia.Rendering` `12.1.1-preview.58` | `Avalonia.ProGpu` | ProGPU/WebGPU rendering backend |
| 12.1.1 | `ProGPU.Avalonia.SilkNet` `12.1.1-preview.58` | `Avalonia.SilkNet` | Cross-platform Silk.NET windowing backend |
| 11.3.20 | `ProGPU.Avalonia.Rendering` `11.3.20-preview.58` | `Avalonia.ProGpu` | Shared-source ProGPU/WebGPU rendering backend |
| 11.3.20 | `ProGPU.Avalonia.SilkNet` `11.3.20-preview.58` | `Avalonia.SilkNet` | Shared-source Silk.NET windowing backend |
| native ABI | `ProGPU.Backend.Native` `0.1.0-preview.58` | `ProGPU.Backend.Native` | Experimental typed C++ renderer host with isolated wgpu-native and provider-resolved Dawn binaries for Linux, macOS, and Windows x64/arm64 |
| native ABI | `ProGPU.Backend.Dawn` `0.1.0-preview.58` | `ProGPU.Backend.Dawn` | Exact-ABI Dawn device, IOSurface shared memory, and timeline fences |
...
use ProGPU `0.1.0-preview.58`. They intentionally use `ProGPU.*` package IDs;
...
Pack the ProGPU `0.1.0-preview.58` portable runtime packages first, then pack
...
1. Tag and publish ProGPU `0.1.0-preview.58`.
2. Confirm the required ProGPU packages are available from NuGet.org.
3. Pack and test both Avalonia integration lanes.
4. Publish the two `12.1.1-preview.58` packages and the two
   `11.3.20-preview.58` packages.
...
<ItemGroup>
  ...
  <PackageReference Include="ProGPU.Avalonia.Rendering" Version="12.1.1-preview.58" />
  <PackageReference Include="ProGPU.Avalonia.SilkNet" Version="12.1.1-preview.58" />
</ItemGroup>
```
...
`11.3.20-preview.58` and pin the Avalonia packages to `11.3.20`.
