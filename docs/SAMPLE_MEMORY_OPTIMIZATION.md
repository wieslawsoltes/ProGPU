...
| Step 30 | Avalonia compact retained commands | Matched Composition + rooted heap + Xcode Instruments | **163,936 → 5,696 B command arrays; 11,521,811 B rooted total** | **5,383 B/frame, 120.27 FPS; native heap+anonymous VM 17.5% below Skia** | Complete |
| Step 31 | Avalonia analytic ellipse recording | Matched Composition + EventPipe + Xcode Instruments | **9,628,724 → 3,585,604 B sampled allocation; path/arc objects eliminated** | **6,225 B/frame vs Skia 7,192 (-13.4%); 119.84 FPS; no Metal errors** | Complete |
...
### Current WinUI native-memory qualification

The current Release `Basic Input` sample was captured with Xcode Allocations
after the Avalonia/ProGPU visual cold-state change. It retained
188,786,048 bytes of native heap plus anonymous VM: 27,633,024 allocator bytes
and 161,153,024 anonymous-VM bytes. End-of-capture IOSurface and
IOAccelerator VM were 32,768,000 and 1,212,416 bytes. The 300-frame process
allocated 683 managed bytes/frame and retained 22,237,088 managed bytes.

This single capture is 22,323,872 bytes below the preceding
211,109,920-byte native-plus-anonymous-VM median, but is a no-growth/leak
qualification rather than a causal performance claim because macOS drawable
and IOAccelerator high-water marks vary between launches. The raw Instruments
trace and XML export were deleted after the compact allocation summary was
written.

### Step 31 — allocation-free ordinary ellipse recording

Status: **complete**.

The final ControlCatalog allocation comparison traced the remaining animated
Composition gap to an avoidable retained path. `DrawEllipse` constructed an
ellipse geometry before testing whether the brush needed a content clip, then
recorded the same shape through the typed analytic-ellipse command. Solid and
gradient brushes do not consume the clip geometry.

Brush classification now precedes geometry construction. Only image and scene
brushes materialize the clip; common solid and gradient ellipses remain typed
GPU primitives. The post-fix 600-frame trace estimated 3,585,604 allocated
bytes, down from 9,628,724, and removed `ArcSegment`, `PathSegment[]`,
`PathFigure[]`, `PathFigure`, and `ProGpuPathShape` from the measured
allocation list. The matched Skia trace estimated 3,791,184 bytes.

Three alternating fresh-process runs measured ProGPU at 119.837 FPS,
9.332 ms mean page p99, and 6,225 bytes/frame. Skia measured 9.291 ms mean
page p99 and 7,192 bytes/frame. ProGPU therefore allocates 13.4% less on this
active custom-composition workload without reducing the animation rate,
custom-visual compilations, draw count, raster quality, or retained-scene
correctness.

The final forced-GC/native capture retained 10,974,455 rooted managed bytes
for ProGPU versus 8,419,453 for Skia, while ProGPU's native allocator payload
was 23,159,856 versus 28,184,000 bytes. ProGPU's active physical footprint was
27,508,688 bytes higher, but it fell by 172,595,610 bytes during the
diagnostic hold as owned-unmapped AGX residency decommitted. IOSurface
residency stayed at 6,553,600 bytes versus Skia's 39,321,600.

The final Xcode capture reported no Metal errors, compiler spills, hangs, or
hang risks in either lane. ProGPU recorded no drawable waits; Skia recorded
92 waits totaling 715.545 ms in the rolling Metal window. Raw Instruments
bundles, exported XML, and scratch totaling 740,861,595 bytes were deleted
after compact summaries were emitted. Evidence is under
