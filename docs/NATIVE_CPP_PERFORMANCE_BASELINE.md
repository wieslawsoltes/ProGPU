...
vertex meshes, and compute-rasterized path fills. The new standalone
...
square/round point batches, indexed/unindexed triangle-list, strip, or fan
meshes, and non-combined path fills are grouped by consecutive family. Path
fills preserve line/quadratic/cubic/analytic-arc segments, both fill rules,
affine transforms, and the shared 4x4/8x8 coverage pipeline. Dot grids retain
...
compiler contract verifies analytic, geometry, path-fill, dot-grid,
point-batch, and mesh
...
commands: 216 solid/linear/radial/two-point-conical/sweep gradient analytic
fills, 48 gradient-stroked lines, 24 gradient dot grids, 32 curved retained
path fills containing 96 segments, 32 point-batch commands containing 96
points, and 32 attributed/indexed vertex meshes covering all
three topologies and all 29 vertex-color blend modes. The managed compiler
lowers the immutable picture once to five retained native draws, six
resources, five brushes, eight gradient stops, and a 42,152-byte pointer-free
stream. The 1,000-frame qualification took `24.3375 ms` and allocated `164,496
bytes` to compile once; the initial C++ scene update took `1.0037 ms` and
allocated `48 bytes`.
...
|---|---:|---:|
| p50 CPU submission | 0.0471 ms | 0.1607 ms |
| p95 CPU submission | 0.1407 ms | 0.3112 ms |
| p50 GPU completion wait | 1.2615 ms | 1.2610 ms |
| p95 GPU completion wait | 3.7952 ms | 3.7963 ms |
| p50 synchronized end to end | 1.3233 ms | 1.4376 ms |
| p95 synchronized end to end | 3.8762 ms | 4.0285 ms |
| managed allocation / stable frame | 0 bytes | 0 bytes |

Native submission p50 is 70.7% lower and p95 is 54.8% lower; synchronized p50
is 8.0% lower and p95 is 3.8% lower. GPU-completion waits are on par because
...
hash `9A202D13386E3978`. The JSON distribution is retained under
...
## Arbitrary and combined retained vector-mask qualification

The Apple M3 Pro/Metal Release gate extends the same 384-primitive matched
picture with six nested geometry clips, including a noncanonical cubic path and
an outer-minus-rounded-rectangle combined path. Canonical one-to-four-node
masks retain the analytic fast path; this workload therefore forces the general
retained vector-mask resource, the canonical three-instruction postfix boolean
program, production path rasterizer, ordered GPU clip composition, and final R8
group mask. Native and managed execute the same ProGPU-owned boolean algorithm
and `PathRasterizer.wgsl`; neither side CPU-flattens the combined geometry.

After 300 alternating warm-up frames, 1,000 synchronized frames measured native
versus managed total p50 at `2.5900/2.8986 ms`, p95 at `3.8714/4.2690 ms`, and
p99 at `4.1045/5.0707 ms`. CPU submission p50 was `0.0729/0.5217 ms`. Both
...
117,248 bytes and the one-time native update allocates only the managed
...
`0.000161555/255`. The JSON and PNG/PPM evidence is retained under
`artifacts/progpu-native/benchmarks/managed-picture-boolean-vector-mask-1000.json`
and
...
are respectively `cdfb10945161232b4af5bce0ae465d2b1ac2f55532494e90b70c155a7ab8be45`,
`1618cef1a4d3cd55762eac9ccb2e89145f42685abe15fceaa3c7f04ee1397a0e`,
`163a2ed28934a03e96d17af64be7841f890b93ae6876e5220028cd44e8386406`,
...
300 path before a 600 by 320 path, subtracts a retained rectangular hole from
the first path with the same three-instruction GPU program, then composes five
clips on a 640 by 360 Dawn/Metal target. Its interior/hole/outside pixel oracle
and second stable render protect the boolean contract, final atlas-denominator
