...
assert.equal(artifact.name, '@wieslawsoltes/progpu');
...
const importMap = { imports: { '@wieslawsoltes/progpu': `./node_modules/${artifact.name}/index.js` } };
...
try {
  ...
  const result = await Promise.race([page.evaluate(async () => {
    ...
    stage('import');
    const { createRenderer, SceneBuilder, Path } = await import('@wieslawsoltes/progpu');
