// Algorithm: Flatten vector curves, bin shape instances into a 16x16 screen grid, traverse their BVHs, and shade queued pixels into the destination texture.
// Time complexity: O(C*S + I + P*log L), where C is curves, S subdivisions per curve, I instances, P covered pixels, and L primitives per shape.
// Space complexity: O(C*S + I + P + G*I), for flattened segments, instances, ray queues, and bounded grid-cell shape indices.
struct BvhNode {
    min_bounds: vec2<f32>,
    max_bounds: vec2<f32>,
