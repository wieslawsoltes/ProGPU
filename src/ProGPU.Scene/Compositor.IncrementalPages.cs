...
public unsafe partial class Compositor
{
    ...
    private readonly Dictionary<Visual, HashSet<IncrementalScenePageKey>>
    ...
    {
        internal required IncrementalScenePageKey Key { get; init; }
        ...
        internal required CompositorDrawCall[] DrawCalls { get; init; }
    ...
    private bool CanUseIncrementalScenePage(Visual node)
    {
        ...
        if (node is not IIncrementalRenderCommandCache)
            return RejectIncrementalScenePage(
        ...
        if (_maskStack.Count != 0 || _maskRenderPasses.Count != 0)
        ...
        in IncrementalScenePageKey key)
    {
        if (!_incrementalScenePageKeysByVisual.TryGetValue(
                node,
                out HashSet<IncrementalScenePageKey>? keys))
        {
            return false;
        }
        ...
        in IncrementalScenePageBoundary boundary)
    {
        ...
        if (!_incrementalScenePageKeysByVisual.TryGetValue(
                ...
                out HashSet<IncrementalScenePageKey>? keys))
        {
            keys = new HashSet<IncrementalScenePageKey>();
    ...
    private static bool TryFindObsoleteIncrementalScenePageKey(
        HashSet<IncrementalScenePageKey> keys,
        long currentContentVersion,
        out IncrementalScenePageKey obsoleteKey)
    {
        ...
        IncrementalScenePage? reusablePage)
    {
        ...
        var drawCalls = CopyIncrementalScenePageArray(
        ...
        {
            CompositorDrawCall drawCall = drawCalls[index];
            drawCall.IndexStart -= drawCall.Type switch
            {
    ...
    private void AppendIncrementalScenePage(IncrementalScenePage page)
    {
        ...
        {
            CompositorDrawCall drawCall = page.DrawCalls[index];
            drawCall.IndexStart += drawCall.Type switch
            {
