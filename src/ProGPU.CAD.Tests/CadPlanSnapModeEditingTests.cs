using ACadSharp;
using ACadSharp.Tables;
using Xunit;

namespace ProGPU.CAD.Tests;

public sealed class CadPlanSnapModeEditingTests
{
    [Fact]
    public void SnapModeCommandUsesOneGenerationAndExactUndoRedo()
    {
        CadDocumentSession session = CadDocumentSession.CreateNew(
            ACadVersion.AC1032);
        var history = new CadDocumentHistory(session);
        Assert.False(session.Read(document =>
            document.VPorts[VPort.DefaultName].SnapOn));

        Assert.Equal(1UL, history.Execute(
            new CadSetPlanSnapModeCommand(true)));
        Assert.True(session.Read(document =>
            document.VPorts[VPort.DefaultName].SnapOn));
        Assert.True(new CadSnapshotCompiler().Compile(session)
            .PlanGridSnapSettings.IsEnabled);

        Assert.True(history.TryUndo(out ulong undone));
        Assert.Equal(2UL, undone);
        Assert.False(session.Read(document =>
            document.VPorts[VPort.DefaultName].SnapOn));
        Assert.True(history.TryRedo(out ulong redone));
        Assert.Equal(3UL, redone);
        Assert.True(session.Read(document =>
            document.VPorts[VPort.DefaultName].SnapOn));

        Assert.Throws<InvalidOperationException>(() => history.Execute(
            new CadSetPlanSnapModeCommand(true)));
        Assert.Equal(redone, session.ContentGeneration);
    }

    [Fact]
    public void SnapModeCommandRejectsInterveningViewportMutation()
    {
        var document = new CadDocument(ACadVersion.AC1032);
        var session = new CadDocumentSession(document);
        var history = new CadDocumentHistory(session);
        history.Execute(new CadSetPlanSnapModeCommand(true));
        document.VPorts[VPort.DefaultName].SnapOn = false;

        Assert.Throws<InvalidOperationException>(() =>
            history.TryUndo(out _));
        Assert.Equal(1UL, session.ContentGeneration);
        Assert.Equal(1, history.UndoCount);
        Assert.False(document.VPorts[VPort.DefaultName].SnapOn);
    }

    [Theory]
    [InlineData(CadDocumentFormat.Dxf)]
    [InlineData(CadDocumentFormat.Dwg)]
    public async Task EditedSnapModeSurvivesDxfAndDwgRoundTrip(
        CadDocumentFormat format)
    {
        CadDocumentSession session = CadDocumentSession.CreateNew(
            ACadVersion.AC1032);
        new CadDocumentHistory(session).Execute(
            new CadSetPlanSnapModeCommand(true));
        using var stream = new MemoryStream();
        var store = new CadDocumentStore();

        await store.SaveAsync(
            session,
            stream,
            format,
            new CadSaveOptions { AllowUncertifiedWrite = true });
        stream.Position = 0;
        CadLoadResult loaded = await store.LoadAsync(
            stream,
            format,
            sourceName: $"snap-mode.{format.ToString().ToLowerInvariant()}");

        Assert.True(loaded.Session.Read(document =>
            document.VPorts[VPort.DefaultName].SnapOn));
        Assert.True(new CadSnapshotCompiler().Compile(loaded.Session)
            .PlanGridSnapSettings.IsEnabled);
    }
}
