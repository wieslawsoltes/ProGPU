...
public sealed class CadShxMTextTests
{
    ...
    public void DynamicManualFinalColumnConsumesRemainingLines(double finalHeight, bool reversed)
    {
        ...
        {
            Style = AddStyle(document), Value = @"AA\PBB\PAA\PBB", Height = 10,
        ...
        CadDocumentSnapshot snapshot = Compile(new CadDocumentSession(document), CreateCache());
        CadShxMTextPrimitive text = Assert.Single(snapshot.ShxMTexts.ToArray());
        Assert.Equal(2, text.ColumnCount);
        ...
        CadShxGlyphInstance[] positions = snapshot.ShxGlyphInstances.ToArray();
