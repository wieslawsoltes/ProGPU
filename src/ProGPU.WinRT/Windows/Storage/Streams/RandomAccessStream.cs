...
/// <summary>
/// Seekable managed-stream adapter used at WinRT-shaped ownership boundaries.
/// Construction and cloning are O(1); reads and writes retain the complexity
/// and allocation behavior of the wrapped stream.
/// </summary>
        ...
        bool leaveOpen = false)
    {
        ...
        if (!stream.CanSeek)
        {
            throw new ArgumentException(
                "A random-access stream must be seekable.",
                nameof(stream));
        }
        _stream = stream;
