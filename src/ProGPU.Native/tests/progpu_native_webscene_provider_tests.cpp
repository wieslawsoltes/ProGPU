...
std::vector<std::byte> create_native_vector_mask_scene_stream() {
    using progpu::native::semantic_scene_builder;
    const auto identity = semantic_scene_builder::identity_transform();
    struct mask_rectangle {
        float x;
        float y;
        float width;
        float height;
    };
    constexpr std::array<mask_rectangle, 5U> rectangles{{
        {120.0F, 30.0F, 400.0F, 300.0F},
        {20.0F, 20.0F, 600.0F, 320.0F},
    ...
    for (std::size_t index = 0; index < rectangles.size(); ++index) {
        ...
        const float right = rectangle.x + rectangle.width;
        const float bottom = rectangle.y + rectangle.height;
        const std::size_t offset = index * 4U;
        segments[offset + 0U] = {{rectangle.x, rectangle.y},
            {right, rectangle.y}, {}, {},
            PROGPU_NATIVE_PATH_SEGMENT_LINE, 0U, 0U, 0U};
        segments[offset + 1U] = {{right, rectangle.y},
            {right, bottom}, {}, {},
            PROGPU_NATIVE_PATH_SEGMENT_LINE, 0U, 0U, 0U};
        segments[offset + 2U] = {{right, bottom},
            {rectangle.x, bottom}, {}, {},
            PROGPU_NATIVE_PATH_SEGMENT_LINE, 0U, 0U, 0U};
        segments[offset + 3U] = {{rectangle.x, bottom},
    ...
    const progpu_native_analytic_primitive content{
        ...
        identity};
    semantic_scene_builder builder(794U, 1U);
    require(builder.reserve(3U, 4U, 2048U),
        "native vector-mask scene reserve failed");
    std::uint32_t brush = PROGPU_NATIVE_SCENE_NO_INDEX;
    std::uint32_t mask = PROGPU_NATIVE_SCENE_NO_INDEX;
    std::uint32_t state_index = PROGPU_NATIVE_SCENE_NO_INDEX;
    require(builder.add_solid_brush(
        {0.0F, 0.85F, 1.0F, 1.0F}, 1.0F, brush),
        "native vector-mask brush recording failed");
    ...
    require(builder.add_state(state, state_index) &&
        ...
        builder.restore(),
        "native vector-mask scene recording failed");
    std::vector<std::byte> stream;
    require(builder.build(stream), "native vector-mask stream build failed");
    return stream;
}
...
using progpu::native::tests::create_semantic_picture_mask_scene_stream;
...
using progpu::native::tests::verify_semantic_state_mask_scene;
using progpu::native::tests::verify_semantic_state_mask_media_scene;
...
void verify_semantic_brush_mask_scene(
    IOSurfaceRef surface,
    const char* output_path) {
    require(surface != nullptr,
        "semantic brush-mask scene has no IOSurface");
    require(IOSurfaceLock(surface, kIOSurfaceLockReadOnly, nullptr) ==
            kIOReturnSuccess,
        "could not lock semantic brush-mask IOSurface");
    const auto* bytes = static_cast<const std::uint8_t*>(
        IOSurfaceGetBaseAddress(surface));
    const std::size_t width = IOSurfaceGetWidth(surface);
    const std::size_t height = IOSurfaceGetHeight(surface);
    ...
    require(bytes != nullptr && width == 64U && height == 48U &&
        row_bytes >= width * 4U,
        "unexpected semantic brush-mask IOSurface storage");
    const auto pixel = [bytes, row_bytes](std::size_t x, std::size_t y) {
        return bytes + y * row_bytes + x * 4U;
    };
    ...
    require(middle[0] >= 42U && middle[0] <= 90U &&
            middle[1] >= 32U && middle[1] <= 80U && middle[2] <= 32U,
        "GPU composite brush mask lost its multiplied gradient midpoint");
    ...
    if (output_path != nullptr && output_path[0] != '\0') {
        std::FILE* output = std::fopen(output_path, "wb");
        require(output != nullptr,
            "could not create semantic brush-mask capture");
        std::fprintf(output, "P6\n%zu %zu\n255\n", width, height);
        for (std::size_t y = 0; y < height; ++y) {
            for (std::size_t x = 0; x < width; ++x) {
                const std::uint8_t* source = pixel(x, y);
                const std::uint8_t rgb[]{
                    source[2], source[1], source[0]};
                require(std::fwrite(rgb, sizeof(rgb), 1U, output) == 1U,
                    "semantic brush-mask capture write failed");
            }
        }
        require(std::fclose(output) == 0,
            "semantic brush-mask capture close failed");
    }
    require(IOSurfaceUnlock(surface, kIOSurfaceLockReadOnly, nullptr) ==
            kIOReturnSuccess,
        "could not unlock semantic brush-mask IOSurface");
}

void verify_semantic_picture_mask_scene(
    IOSurfaceRef surface,
    const char* output_path) {
    require(surface != nullptr,
        "semantic picture-mask scene has no IOSurface");
    require(IOSurfaceLock(surface, kIOSurfaceLockReadOnly, nullptr) ==
            kIOReturnSuccess,
        "could not lock semantic picture-mask IOSurface");
    const auto* bytes = static_cast<const std::uint8_t*>(
        IOSurfaceGetBaseAddress(surface));
    const std::size_t width = IOSurfaceGetWidth(surface);
    const std::size_t height = IOSurfaceGetHeight(surface);
    ...
    require(bytes != nullptr && width == 64U && height == 48U &&
        row_bytes >= width * 4U,
        "unexpected semantic picture-mask IOSurface storage");
    const auto pixel = [bytes, row_bytes](std::size_t x, std::size_t y) {
        return bytes + y * row_bytes + x * 4U;
    };
    const auto cyan = [](const std::uint8_t* value) {
    ...
    require(cyan(pixel(16U, 24U)),
        "retained picture mask lost its left opaque region");
    require(clear(pixel(32U, 24U)),
        "retained picture mask did not preserve its transparent gap");
    require(cyan(pixel(48U, 24U)),
        "retained picture mask lost its right opaque region");
    require(clear(pixel(4U, 24U)),
        "retained picture mask escaped its explicit bounds");
    if (output_path != nullptr && output_path[0] != '\0') {
        std::FILE* output = std::fopen(output_path, "wb");
        require(output != nullptr,
            "could not create semantic picture-mask capture");
        std::fprintf(output, "P6\n%zu %zu\n255\n", width, height);
        for (std::size_t y = 0; y < height; ++y) {
            for (std::size_t x = 0; x < width; ++x) {
                const std::uint8_t* source = pixel(x, y);
                const std::uint8_t rgb[]{
                    source[2], source[1], source[0]};
                require(std::fwrite(rgb, sizeof(rgb), 1U, output) == 1U,
                    "semantic picture-mask capture write failed");
            }
        }
        require(std::fclose(output) == 0,
            "semantic picture-mask capture close failed");
    }
    require(IOSurfaceUnlock(surface, kIOSurfaceLockReadOnly, nullptr) ==
            kIOReturnSuccess,
        "could not unlock semantic picture-mask IOSurface");
}

void verify_semantic_vector_mask_scene(IOSurfaceRef surface) {
    require(surface != nullptr,
        "semantic vector-mask scene has no IOSurface");
    require(IOSurfaceLock(surface, kIOSurfaceLockReadOnly, nullptr) ==
            kIOReturnSuccess,
        "could not lock semantic vector-mask IOSurface");
    const auto* bytes = static_cast<const std::uint8_t*>(
        IOSurfaceGetBaseAddress(surface));
    const std::size_t width = IOSurfaceGetWidth(surface);
    const std::size_t height = IOSurfaceGetHeight(surface);
    const std::size_t row_bytes = IOSurfaceGetBytesPerRow(surface);
    require(bytes != nullptr && width == 640U && height == 360U &&
        row_bytes >= width * 4U,
        "unexpected semantic vector-mask IOSurface storage");
    const auto pixel = [bytes, row_bytes](std::size_t x, std::size_t y) {
        return bytes + y * row_bytes + x * 4U;
    };
    const auto cyan = [](const std::uint8_t* value) {
        return value[0] >= 240U && value[1] >= 190U &&
            value[2] <= 24U && value[3] >= 240U;
    };
    const auto clear = [](const std::uint8_t* value) {
...
int main(int argc, char** argv) {
    ...
    canvas = api.create_canvas(
        provider, &canvas_configuration, 64U, 48U);
    require(canvas != nullptr,
        "semantic state-mask canvas creation failed");
    texture_handle = 0U;
    require(api.acquire(provider, canvas, &texture_handle) ==
            WEBSCENE_GPU_STATUS_SUCCESS && texture_handle != 0U,
        "semantic state-mask texture acquisition failed");
    texture = reinterpret_cast<WGPUTexture>(texture_handle);
    view = resolve<WGPUProcTextureCreateView>(
        api, provider, "wgpuTextureCreateView")(
        texture, &view_descriptor);
    require(view != nullptr,
        "semantic state-mask target view creation failed");
    auto state_mask_scene =
        create_semantic_state_mask_scene_stream(64U, 48U);
    scene_metrics = {};
    scene_metrics.struct_size = sizeof(scene_metrics);
    require(progpu_native_engine_update_scene(
        engine,
        ...
        scene_metrics.draw_count == 1U,
        "semantic state-mask scene update failed");
    progpu_native_scene_frame state_mask_frame = semantic_frame;
    state_mask_frame.target_view =
        reinterpret_cast<std::uintptr_t>(view);
    state_mask_frame.scene_id = 102U;
    state_mask_frame.generation = 1U;
    semantic_metrics = {};
    semantic_metrics.struct_size = sizeof(semantic_metrics);
    require(progpu_native_engine_render_scene(
        engine,
        ...
        "semantic state-mask rendering failed");
    semantic_layer_metrics = {};
    semantic_layer_metrics.struct_size = sizeof(semantic_layer_metrics);
    require(progpu_native_engine_get_layer_metrics(
        engine,
    ...
    require(progpu_native_engine_render_scene(
        ...
        semantic_metrics.texture_upload_bytes == 0U &&
        semantic_metrics.uniform_upload_bytes == 0U,
        "stable semantic state-mask replay rebuilt retained resources");
    semantic_layer_metrics = {};
    semantic_layer_metrics.struct_size = sizeof(semantic_layer_metrics);
    require(progpu_native_engine_get_layer_metrics(
        engine,
        ...
        semantic_layer_metrics.cache_hit == 1U &&
        semantic_layer_metrics.mask_bind_group_generation ==
            state_mask_bind_group_generation &&
        semantic_layer_metrics.mask_uniform_upload_bytes == 0U,
        "stable semantic state-mask metrics did not retain resources");
    std::uint64_t state_mask_submission{};
    require(progpu_native_engine_get_last_submission(
        engine,
        &state_mask_submission) == PROGPU_NATIVE_STATUS_SUCCESS,
        "semantic state-mask submission token unavailable");
    std::uint8_t state_mask_complete{};
    require(progpu_native_engine_poll_submission(
        engine,
        state_mask_submission,
        1U,
        &state_mask_complete) == PROGPU_NATIVE_STATUS_SUCCESS &&
        state_mask_complete != 0U,
        "semantic state-mask scene did not reach GPU completion");
    resolve<WGPUProcTextureViewRelease>(
        api, provider, "wgpuTextureViewRelease")(view);
    resolve<WGPUProcTextureRelease>(
        api, provider, "wgpuTextureRelease")(texture);
    webscene_gpu_external_texture state_mask_external{};
    state_mask_external.struct_size = sizeof(state_mask_external);
    require(api.present(provider, canvas, &state_mask_external) ==
            WEBSCENE_GPU_STATUS_SUCCESS &&
        state_mask_external.handle_kind ==
            WEBSCENE_GPU_HANDLE_IOSURFACE &&
        (state_mask_external.flags &
            WEBSCENE_GPU_EXTERNAL_TEXTURE_GPU_COMPLETE) != 0U,
        "semantic state-mask presentation failed");
    verify_semantic_state_mask_scene(
        reinterpret_cast<IOSurfaceRef>(
            state_mask_external.shared_handle),
        "progpu-native-semantic-state-mask.ppm");
    api.release_external(provider, &state_mask_external);
    api.destroy_canvas(provider, canvas);

    canvas = api.create_canvas(
        provider, &canvas_configuration, 64U, 48U);
    require(canvas != nullptr,
        "semantic state-mask media canvas creation failed");
    texture_handle = 0U;
    require(api.acquire(provider, canvas, &texture_handle) ==
            WEBSCENE_GPU_STATUS_SUCCESS && texture_handle != 0U,
        "semantic state-mask media texture acquisition failed");
    texture = reinterpret_cast<WGPUTexture>(texture_handle);
    view = resolve<WGPUProcTextureCreateView>(
        api, provider, "wgpuTextureCreateView")(
        texture, &view_descriptor);
    require(view != nullptr,
        "semantic state-mask media target view creation failed");
    auto state_mask_media_scene =
        create_semantic_state_mask_media_scene_stream(64U, 48U);
    scene_metrics = {};
    scene_metrics.struct_size = sizeof(scene_metrics);
    require(progpu_native_engine_update_scene(
        engine,
        ...
        scene_metrics.resource_count == 4U &&
        scene_metrics.draw_count == 2U,
        "semantic state-mask media scene update failed");
    progpu_native_scene_frame state_mask_media_frame = semantic_frame;
    state_mask_media_frame.target_view =
        reinterpret_cast<std::uintptr_t>(view);
    state_mask_media_frame.scene_id = 103U;
    state_mask_media_frame.generation = 1U;
    semantic_metrics = {};
    semantic_metrics.struct_size = sizeof(semantic_metrics);
    require(progpu_native_engine_render_scene(
        engine,
        ...
        semantic_metrics.submission_count == 1U &&
        semantic_metrics.texture_upload_bytes == 96U &&
        semantic_metrics.color_glyph_upload_bytes == 16U &&
        semantic_metrics.uniform_upload_bytes >= 24U * sizeof(float),
        "semantic state-mask media rendering failed");
    semantic_layer_metrics = {};
    semantic_layer_metrics.struct_size = sizeof(semantic_layer_metrics);
    require(progpu_native_engine_get_layer_metrics(
        engine,
        ...
        semantic_layer_metrics.mask_uniform_upload_bytes ==
            24U * sizeof(float),
        "semantic state-mask media metrics are incorrect");
    const std::uint32_t state_mask_media_bind_group_generation =
        semantic_layer_metrics.mask_bind_group_generation;
    semantic_metrics = {};
    semantic_metrics.struct_size = sizeof(semantic_metrics);
    require(progpu_native_engine_render_scene(
        ...
        semantic_metrics.uniform_upload_bytes == 0U &&
        semantic_metrics.color_glyph_upload_bytes == 0U,
        "stable semantic state-mask media replay rebuilt resources");
    semantic_layer_metrics = {};
    semantic_layer_metrics.struct_size = sizeof(semantic_layer_metrics);
    require(progpu_native_engine_get_layer_metrics(
        engine,
        ...
        semantic_layer_metrics.cache_hit == 1U &&
        semantic_layer_metrics.mask_bind_group_generation ==
            state_mask_media_bind_group_generation &&
        semantic_layer_metrics.mask_uniform_upload_bytes == 0U,
        "stable semantic state-mask media metrics did not retain resources");
    std::uint64_t state_mask_media_submission{};
    require(progpu_native_engine_get_last_submission(
        engine,
        &state_mask_media_submission) == PROGPU_NATIVE_STATUS_SUCCESS,
        "semantic state-mask media submission token unavailable");
    std::uint8_t state_mask_media_complete{};
    require(progpu_native_engine_poll_submission(
        engine,
        state_mask_media_submission,
        1U,
        &state_mask_media_complete) == PROGPU_NATIVE_STATUS_SUCCESS &&
        state_mask_media_complete != 0U,
        "semantic state-mask media scene did not reach GPU completion");
    resolve<WGPUProcTextureViewRelease>(
        api, provider, "wgpuTextureViewRelease")(view);
    resolve<WGPUProcTextureRelease>(
        api, provider, "wgpuTextureRelease")(texture);
    webscene_gpu_external_texture state_mask_media_external{};
    ...
    require(api.present(provider, canvas, &state_mask_media_external) ==
        ...
        (state_mask_media_external.flags &
            WEBSCENE_GPU_EXTERNAL_TEXTURE_GPU_COMPLETE) != 0U,
        "semantic state-mask media presentation failed");
    ...
    canvas = api.create_canvas(
        provider, &canvas_configuration, 64U, 48U);
    require(canvas != nullptr,
        "semantic brush-mask canvas creation failed");
    texture_handle = 0U;
    require(api.acquire(provider, canvas, &texture_handle) ==
            WEBSCENE_GPU_STATUS_SUCCESS && texture_handle != 0U,
        "semantic brush-mask texture acquisition failed");
    texture = reinterpret_cast<WGPUTexture>(texture_handle);
    view = resolve<WGPUProcTextureCreateView>(
        api, provider, "wgpuTextureCreateView")(
        texture, &view_descriptor);
    ...
    require(progpu_native_engine_update_scene(
        ...
        scene_metrics.resource_count == 2U &&
        scene_metrics.draw_count == 1U,
        "semantic brush-mask scene update failed");
    progpu_native_scene_frame brush_mask_frame = semantic_frame;
    brush_mask_frame.target_view =
        reinterpret_cast<std::uintptr_t>(view);
    brush_mask_frame.scene_id = 107U;
    ...
    require(progpu_native_engine_get_layer_metrics(
        ...
        semantic_layer_metrics.mask_uniform_upload_bytes >=
            24U * sizeof(float),
        "semantic GPU composite-mask metrics are incorrect");
    semantic_metrics = {};
    semantic_metrics.struct_size = sizeof(semantic_metrics);
    require(progpu_native_engine_render_scene(
        engine,
        ...
        semantic_metrics.vertex_upload_bytes == 0U &&
        semantic_metrics.uniform_upload_bytes == 0U,
        "stable semantic GPU brush-mask replay rebuilt resources");
    std::uint64_t brush_mask_submission{};
    require(progpu_native_engine_get_last_submission(
        engine,
        &brush_mask_submission) == PROGPU_NATIVE_STATUS_SUCCESS,
        "semantic brush-mask submission token unavailable");
    std::uint8_t brush_mask_complete{};
    require(progpu_native_engine_poll_submission(
        engine,
        brush_mask_submission,
        1U,
        &brush_mask_complete) == PROGPU_NATIVE_STATUS_SUCCESS &&
        brush_mask_complete != 0U,
        "semantic brush-mask scene did not reach GPU completion");
    resolve<WGPUProcTextureViewRelease>(
        api, provider, "wgpuTextureViewRelease")(view);
    resolve<WGPUProcTextureRelease>(
        api, provider, "wgpuTextureRelease")(texture);
    webscene_gpu_external_texture brush_mask_external{};
    brush_mask_external.struct_size = sizeof(brush_mask_external);
    require(api.present(provider, canvas, &brush_mask_external) ==
            WEBSCENE_GPU_STATUS_SUCCESS &&
        brush_mask_external.handle_kind ==
            WEBSCENE_GPU_HANDLE_IOSURFACE &&
        (brush_mask_external.flags &
            WEBSCENE_GPU_EXTERNAL_TEXTURE_GPU_COMPLETE) != 0U,
        "semantic brush-mask presentation failed");
    verify_semantic_brush_mask_scene(
        reinterpret_cast<IOSurfaceRef>(
            brush_mask_external.shared_handle),
        "progpu-native-semantic-brush-mask.ppm");
    api.release_external(provider, &brush_mask_external);
    api.destroy_canvas(provider, canvas);

    canvas = api.create_canvas(
        provider, &canvas_configuration, 64U, 48U);
    require(canvas != nullptr,
        "semantic picture-mask canvas creation failed");
    texture_handle = 0U;
    require(api.acquire(provider, canvas, &texture_handle) ==
            WEBSCENE_GPU_STATUS_SUCCESS && texture_handle != 0U,
        "semantic picture-mask texture acquisition failed");
    texture = reinterpret_cast<WGPUTexture>(texture_handle);
    view = resolve<WGPUProcTextureCreateView>(
        api, provider, "wgpuTextureCreateView")(
        texture, &view_descriptor);
    require(view != nullptr,
        "semantic picture-mask target view creation failed");
    auto picture_mask_scene =
        create_semantic_picture_mask_scene_stream(64U, 48U);
    scene_metrics = {};
    scene_metrics.struct_size = sizeof(scene_metrics);
    require(progpu_native_engine_update_scene(
        engine,
        ...
        scene_metrics.resource_count == 2U &&
        scene_metrics.draw_count == 1U,
        "semantic picture-mask scene update failed");
    progpu_native_scene_frame picture_mask_frame = semantic_frame;
    picture_mask_frame.target_view =
        reinterpret_cast<std::uintptr_t>(view);
    picture_mask_frame.scene_id = 108U;
    picture_mask_frame.generation = 1U;
    semantic_metrics = {};
    semantic_metrics.struct_size = sizeof(semantic_metrics);
    require(progpu_native_engine_render_scene(
        engine,
        ...
        semantic_metrics.uniform_upload_bytes >=
            28U * sizeof(float),
        "semantic retained picture-mask rendering failed");
    semantic_layer_metrics = {};
    semantic_layer_metrics.struct_size = sizeof(semantic_layer_metrics);
    require(progpu_native_engine_get_layer_metrics(
        engine,
        ...
        semantic_layer_metrics.mask_uniform_upload_bytes >=
            28U * sizeof(float),
        "semantic retained picture-mask metrics are incorrect");
    semantic_metrics = {};
    semantic_metrics.struct_size = sizeof(semantic_metrics);
    require(progpu_native_engine_render_scene(
        engine,
    ...
    require(progpu_native_engine_get_last_submission(
        engine,
        &picture_mask_submission) == PROGPU_NATIVE_STATUS_SUCCESS,
        "semantic picture-mask submission token unavailable");
    std::uint8_t picture_mask_complete{};
    require(progpu_native_engine_poll_submission(
        engine,
        picture_mask_submission,
        1U,
        &picture_mask_complete) == PROGPU_NATIVE_STATUS_SUCCESS &&
        picture_mask_complete != 0U,
        "semantic picture-mask scene did not reach GPU completion");
    resolve<WGPUProcTextureViewRelease>(
        api, provider, "wgpuTextureViewRelease")(view);
    resolve<WGPUProcTextureRelease>(
        api, provider, "wgpuTextureRelease")(texture);
    webscene_gpu_external_texture picture_mask_external{};
    picture_mask_external.struct_size = sizeof(picture_mask_external);
    require(api.present(provider, canvas, &picture_mask_external) ==
            WEBSCENE_GPU_STATUS_SUCCESS &&
        picture_mask_external.handle_kind ==
            WEBSCENE_GPU_HANDLE_IOSURFACE &&
        (picture_mask_external.flags &
            WEBSCENE_GPU_EXTERNAL_TEXTURE_GPU_COMPLETE) != 0U,
        "semantic picture-mask presentation failed");
    verify_semantic_picture_mask_scene(
        reinterpret_cast<IOSurfaceRef>(
            picture_mask_external.shared_handle),
        "progpu-native-semantic-picture-mask.ppm");
    api.release_external(provider, &picture_mask_external);
    api.destroy_canvas(provider, canvas);

    canvas = api.create_canvas(
        provider, &canvas_configuration, 640U, 360U);
    require(canvas != nullptr,
        "semantic vector-mask canvas creation failed");
    texture_handle = 0U;
    require(api.acquire(provider, canvas, &texture_handle) ==
            WEBSCENE_GPU_STATUS_SUCCESS && texture_handle != 0U,
        "semantic vector-mask texture acquisition failed");
    texture = reinterpret_cast<WGPUTexture>(texture_handle);
    view = resolve<WGPUProcTextureCreateView>(
        api, provider, "wgpuTextureCreateView")(
        texture, &view_descriptor);
    require(view != nullptr,
        "semantic vector-mask target view creation failed");
    auto vector_mask_scene = create_native_vector_mask_scene_stream();
    scene_metrics = {};
    scene_metrics.struct_size = sizeof(scene_metrics);
    require(progpu_native_engine_update_scene(
        engine,
        ...
        scene_metrics.resource_count == 4U &&
        scene_metrics.draw_count == 1U,
        "semantic vector-mask scene update failed");
    progpu_native_scene_frame vector_mask_frame = semantic_frame;
    vector_mask_frame.target_view =
        reinterpret_cast<std::uintptr_t>(view);
    vector_mask_frame.scene_id = 794U;
    vector_mask_frame.generation = 1U;
    vector_mask_frame.width = 640U;
    vector_mask_frame.height = 360U;
    semantic_metrics = {};
    semantic_metrics.struct_size = sizeof(semantic_metrics);
    require(progpu_native_engine_render_scene(
        engine,
        ...
        semantic_metrics.texture_upload_bytes == 0U &&
        semantic_metrics.uniform_upload_bytes >= 24U * sizeof(float),
        "semantic vector-mask rendering failed");
    semantic_layer_metrics = {};
    semantic_layer_metrics.struct_size = sizeof(semantic_layer_metrics);
    require(progpu_native_engine_get_layer_metrics(
        engine,
        ...
        semantic_layer_metrics.mask_uniform_upload_bytes ==
            24U * sizeof(float),
        "semantic vector-mask metrics are incorrect");
    semantic_metrics = {};
    semantic_metrics.struct_size = sizeof(semantic_metrics);
    require(progpu_native_engine_render_scene(
        engine,
        ...
        semantic_metrics.vertex_upload_bytes == 0U &&
        semantic_metrics.uniform_upload_bytes == 0U,
        "stable semantic vector-mask replay rebuilt resources");
    std::uint64_t vector_mask_submission{};
    require(progpu_native_engine_get_last_submission(
        engine,
        &vector_mask_submission) == PROGPU_NATIVE_STATUS_SUCCESS,
        "semantic vector-mask submission token unavailable");
    std::uint8_t vector_mask_complete{};
    require(progpu_native_engine_poll_submission(
        engine,
        vector_mask_submission,
        1U,
        &vector_mask_complete) == PROGPU_NATIVE_STATUS_SUCCESS &&
        vector_mask_complete != 0U,
        "semantic vector-mask scene did not reach GPU completion");
    resolve<WGPUProcTextureViewRelease>(
        api, provider, "wgpuTextureViewRelease")(view);
    resolve<WGPUProcTextureRelease>(
        api, provider, "wgpuTextureRelease")(texture);
    webscene_gpu_external_texture vector_mask_external{};
    vector_mask_external.struct_size = sizeof(vector_mask_external);
    require(api.present(provider, canvas, &vector_mask_external) ==
            WEBSCENE_GPU_STATUS_SUCCESS &&
        vector_mask_external.handle_kind ==
            WEBSCENE_GPU_HANDLE_IOSURFACE &&
        (vector_mask_external.flags &
            WEBSCENE_GPU_EXTERNAL_TEXTURE_GPU_COMPLETE) != 0U,
        "semantic vector-mask presentation failed");
    verify_semantic_vector_mask_scene(
        reinterpret_cast<IOSurfaceRef>(
            vector_mask_external.shared_handle));
    api.release_external(provider, &vector_mask_external);
    api.destroy_canvas(provider, canvas);
