...
inline bool build_mil_image_brush_fixture(std::vector<std::byte>& scene,
    ...
    packet(batch, command::channel_create_resource, 3U, 95U);
    ...
    packet(batch, command::channel_create_resource, 5U, 80U);
    ...
    packet(batch, command::image_brush, 5U, options.opacity,
    ...
    if (progpu_native_mil_channel_set_bitmap_source_rgba8_with_dpi(raw,
