...
bool semantic_scene_builder_records_retained_3d_families() {
    ...
    const bool edge_only_valid = edge_only_builder.draw_meshes_3d(
        ...
        scene::validate(stream.data(), stream.size()).status ==
            PROGPU_NATIVE_STATUS_SUCCESS;
    auto brush_mesh = mesh;
    brush_mesh.flags &= ~(static_cast<std::uint32_t>(
        PROGPU_NATIVE_MESH_3D_MATERIAL_IMAGE) |
        PROGPU_NATIVE_MESH_3D_TILING_MASK);
    brush_mesh.material_image_resource_index = 0U;
    brush_mesh.material_factors = 0U;
    ...
    if (!material_builder.draw_meshes_3d(
            std::span<const progpu_native_scene_mesh_3d>(&brush_mesh, 1U),
            vertices,
            indices,
            lights,
