...
class portable_rounded_rectangle_geometry final :
    public rounded_rectangle_geometry {
public:
    portable_rounded_rectangle_geometry(
        factory* owner,
        ...
        void** value) noexcept override
    {
        ...
        *value = nullptr;
        if (com::guid_equal(interface_id, com::unknown_interface_id()) ||
            ...
            com::guid_equal(
                interface_id,
                rounded_rectangle_geometry_interface_id)) {
            *value = static_cast<rounded_rectangle_geometry*>(this);
            AddRef();
            return com::ok;
        }
        ...
        *contains = 0;
        ...
        const com::result result =
            core::rounded_rectangle_fill_contains_point(
                value_, point, transform, tolerance, &core_contains);
    ...
    void PROGPU_NATIVE_COM_CALL GetRoundedRect(
        rounded_rectangle* value) const noexcept override
    {
        if (value != nullptr) {
            *value = value_;
        }
...
private:
    friend class com::atomic_reference_count<
        portable_rounded_rectangle_geometry>;
    ~portable_rounded_rectangle_geometry() = default;

    com::atomic_reference_count<portable_rounded_rectangle_geometry>
        reference_count_;
    com::pointer<factory> owner_;
    rounded_rectangle value_{};
    com::pointer<path_geometry> path_;
};

} // namespace

com::result create_rounded_rectangle_geometry(
    factory* owner,
    const rounded_rectangle* value,
    rounded_rectangle_geometry** geometry_value) noexcept
{
    ...
    *geometry_value = nullptr;
    if (owner == nullptr || value == nullptr ||
        !core::valid_rounded_rectangle(*value)) {
    ...
    point_2f start{};
    std::array<point_2f, 4U> line_ends{};
    std::array<bezier_segment, 4U> corners{};
    result = core::rounded_rectangle_to_path(
        *value, &start, &line_ends, &corners);
    if (com::failed(result)) {
    ...
    auto* created = new (std::nothrow) portable_rounded_rectangle_geometry(
