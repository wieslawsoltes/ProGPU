...
public sealed class SamplePerformanceRegressionTests
{
    ...
    [Fact]
    public void DensePathAtlasGrowthAlternatesAxesWithoutMovingResidentCoverage()
    {
        using var pathAtlas = new PathAtlas(
            HeadlessWindow.Shared.Context,
            atlasSize: 2048);
        ...
        pathAtlas.RasterizePendingPaths();
        byte[] initialPixels = pathAtlas.AtlasTexture.ReadPixels();
        uint localX = checked((uint)(20 - (int)first.MinX));
        uint localY = checked((uint)(20 - (int)first.MinY));
        int initialCoverageOffset = checked((int)(
            (first.Y + localY) * pathAtlas.AtlasWidth + first.X + localX));
        byte expectedCoverage = initialPixels[initialCoverageOffset];
        Assert.True(expectedCoverage > 200);

        for (int index = 1;
             index < 200 && pathAtlas.AtlasGrowthCount < 2;
             index++)
        {
            _ = pathAtlas.GetOrCreatePath(
        ...
        PathAtlas.PathInfo repeated = pathAtlas.GetOrCreatePath(
            PrimitivePathGeometry.CreateRectangle(0f, 0f, 72f, 72f),
            scale: 1f);
        Assert.Equal(2u, pathAtlas.AtlasGrowthCount);
        Assert.Equal(1024u, pathAtlas.AtlasWidth);
        Assert.Equal(1024u, pathAtlas.AtlasHeight);
        Assert.Equal(1024UL * 1024UL, pathAtlas.PersistentTextureBytes);
        Assert.Equal(first.X, repeated.X);
        Assert.Equal(first.Y, repeated.Y);
        byte[] grownPixels = pathAtlas.AtlasTexture.ReadPixels();
        int grownCoverageOffset = checked((int)(
            (repeated.Y + localY) * pathAtlas.AtlasWidth +
            repeated.X +
            localX));
        Assert.Equal(expectedCoverage, grownPixels[grownCoverageOffset]);
    }

    [Fact]
    public void PathAtlasCompactsStaleFramesBeforeGrowingRecoveryTexture()
    {
        using var pathAtlas = new PathAtlas(
            HeadlessWindow.Shared.Context,
            atlasSize: 2048);
        ...
        pathAtlas.ResetForRenderRetry();
        pathAtlas.RasterizePendingPaths();
        uint initialWidth = pathAtlas.AtlasWidth;
        uint initialHeight = pathAtlas.AtlasHeight;
        uint initialGrowths = pathAtlas.AtlasGrowthCount;
        pathAtlas.CleanupFrame();

        PathGeometry? deferredPath = null;
        for (int index = 0;
             index < 80 && pathAtlas.AtlasAvoidedGrowthCount == 0;
             index++)
        {
            deferredPath = PrimitivePathGeometry.CreateRectangle(
                index * 100f,
                1000f,
                72f,
                72f);
            _ = pathAtlas.GetOrCreatePath(deferredPath, scale: 1f);
        }

        Assert.NotNull(deferredPath);
        Assert.True(pathAtlas.CapacityExceeded);
        Assert.Equal(1u, pathAtlas.AtlasAvoidedGrowthCount);
        Assert.Equal(initialGrowths, pathAtlas.AtlasGrowthCount);
        Assert.Equal(initialWidth, pathAtlas.AtlasWidth);
        Assert.Equal(initialHeight, pathAtlas.AtlasHeight);

        for (int index = 80; index < 90; index++)
        {
            _ = pathAtlas.GetOrCreatePath(
                PrimitivePathGeometry.CreateRectangle(
                    index * 100f,
        ...
        pathAtlas.ResetForRenderRetry();
        PathAtlas.PathInfo recovered =
            pathAtlas.GetOrCreatePath(deferredPath!, scale: 1f);
        Assert.False(pathAtlas.CapacityExceeded);
        Assert.True(recovered.Width > 0);
        Assert.True(recovered.Height > 0);
        pathAtlas.RasterizePendingPaths();
        Assert.Contains(
            pathAtlas.AtlasTexture.ReadPixels(),
            static coverage => coverage > 0);
    }
    ...
    public void UncappedApplicationLoopHasNoMandatorySleep()
    {
        ...
        Assert.Contains("if (!presentedAnyFrame)", appRunner, StringComparison.Ordinal);
