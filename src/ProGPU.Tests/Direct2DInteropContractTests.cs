...
public sealed class Direct2DInteropContractTests
{
    ...
    public void PortableRectangleCoreIsSharedByWindowsComAdapter()
    {
        ...
        Assert.Contains("direct2d_compat::detail::create_drawing_state_block1", provider, StringComparison.Ordinal);
        ...
        Assert.Contains("path->GetWidenedBounds", provider, StringComparison.Ordinal);
        Assert.Contains("path->StrokeContainsPoint", provider, StringComparison.Ordinal);
        Assert.Contains("path->Tessellate", provider, StringComparison.Ordinal);
        Assert.Contains("path->CompareWithGeometry", provider, StringComparison.Ordinal);
        Assert.Contains("path->CombineWithGeometry", provider, StringComparison.Ordinal);
        Assert.Contains("path->Outline", provider, StringComparison.Ordinal);
        ...
        Assert.Contains("ProGPU nested path outline diverged from system Direct2D", providerTest, StringComparison.Ordinal);
        Assert.Contains("ProGPU nested path tessellation diverged from system Direct2D", providerTest, StringComparison.Ordinal);
        Assert.Contains("ProGPU path stroke queries diverged from system Direct2D", providerTest, StringComparison.Ordinal);
        ...
        Assert.Contains("ProGPU drawing-state block1 diverged from system Direct2D", providerTest, StringComparison.Ordinal);
        Assert.Contains("ProGPU base mutation did not preserve drawing-state block1 fields", providerTest, StringComparison.Ordinal);
    ...
    public void PortableDirect2DCompatFactoryPreservesComShapeAndFailsClosed()
    {
        ...
        string stableHeader = ReadRepoFile(
            "src",
            "ProGPU.Native",
            "include",
            "progpu_native.h");
        string semanticImageSource = ReadRepoFile(
            "src",
            "ProGPU.Native",
            "src",
            "Scene",
            "progpu_native_semantic_image.cpp");
        string textureShader = ReadRepoFile(
            "src",
            "ProGPU.Backend",
            "Shaders",
            "Texture.wgsl");
        ...
        Assert.Contains("source.a = 1.0", textureShader, StringComparison.Ordinal);
        ...
        Assert.Contains("bridge_polygon_hole", pathSource, StringComparison.Ordinal);
