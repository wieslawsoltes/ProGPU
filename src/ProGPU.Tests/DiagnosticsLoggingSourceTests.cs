...
public class DiagnosticsLoggingSourceTests
{
    ...
    public void WgpuContextFirstActiveLookupAvoidsSnapshotArrays()
    {
        ...
        string avaloniaDrawingContext = ReadSource(
            "src",
        ...
        Assert.Contains("WgpuContext.TryGetActiveContextForSurface(surfacePtr, out var context)", avaloniaDrawingContext, StringComparison.Ordinal);
        Assert.Contains("WgpuContext.TryGetFirstActiveContext(out current);", avaloniaDrawingContext, StringComparison.Ordinal);
        Assert.DoesNotContain("WgpuContext.ActiveContexts", avaloniaDrawingContext, StringComparison.Ordinal);
    ...
    public void VisualOwnerNotificationsUsePooledSnapshots()
    {
        ...
        Assert.Contains("private Dictionary<string, CompositionAnimation>? _activeAnimations;", source, StringComparison.Ordinal);
        Assert.Contains("StringComparer.OrdinalIgnoreCase))[propertyName] = animation;", source, StringComparison.Ordinal);
        Assert.Contains("if (_activeAnimations is not { Count: > 0 } activeAnimations)", source, StringComparison.Ordinal);
    ...
    [Fact]
    public void PathAtlasCleanupAndRasterizationUsePooledBuffers()
    {
        ...
        Assert.Contains("_atlasTexture.ClearRenderTarget();", pathAtlas, StringComparison.Ordinal);
        Assert.DoesNotContain("byte[] clearData = new byte[_atlasSize * _atlasSize * 4];", pathAtlas, StringComparison.Ordinal);
        Assert.Contains("ArrayPool<PendingRasterization>.Shared.Rent", pathAtlas, StringComparison.Ordinal);
        Assert.Contains("ArrayPool<RasterizationDispatch>.Shared.Rent", pathAtlas, StringComparison.Ordinal);
        Assert.Contains("ArrayPool<byte>.Shared.Rent(totalUniformBytes)", pathAtlas, StringComparison.Ordinal);
        Assert.Contains("nint[]? bindGroupsToRelease = null;", pathAtlas, StringComparison.Ordinal);
        Assert.Contains("PooledRemovalBuffer.Add(", pathAtlas, StringComparison.Ordinal);
        Assert.Contains("ref bindGroupsToRelease", pathAtlas, StringComparison.Ordinal);
        ...
        Assert.Contains("private readonly BindGroupLayout* _computeBindGroupLayout;", pathAtlas, StringComparison.Ordinal);
        ...
        Assert.Contains("var segments = new List<GpuPathSegment>(EstimateSegmentCapacity(figures, fillOnly));", pathAtlas, StringComparison.Ordinal);
        ...
        Assert.Contains("private static int EstimateSegmentCapacity(List<PathFigure> figures, bool fillOnly)", pathAtlas, StringComparison.Ordinal);
