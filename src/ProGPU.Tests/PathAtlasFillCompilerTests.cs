...
public class PathAtlasFillCompilerTests
{
    ...
    [Fact]
    public void RationalCubicFillPreservesCanonicalWeightsWithoutFlattening()
    {
        var path = new PathGeometry();
        ...
        Assert.Equal(2u, Assert.Single(records).SegmentCount);
        Assert.Equal(2, segments.Length);
        Assert.Equal(5u, segments[0].SegmentType);
        Assert.Equal(Vector2.Zero, segments[0].P0);
        Assert.Equal(new Vector2(0f, 10f), segments[0].P1);
        Assert.Equal(new Vector2(10f, 10f), segments[0].P2);
        Assert.Equal(new Vector2(10f, 0f), segments[0].P3);
        Assert.Equal(0.5f, BitConverter.UInt32BitsToSingle(segments[0].Pad0));
        Assert.Equal(1.5f, BitConverter.UInt32BitsToSingle(segments[0].Pad1));
        Assert.Equal(0u, segments[0].Pad2);
        Assert.Equal((0f, 0f, 10f, 10f), (minX, minY, maxX, maxY));
    }

    [Fact]
    public void RationalCubicFillRejectsInvalidWeightsAndBooleanReconstruction()
    {
        var invalid = new PathGeometry();
        var invalidFigure = new PathFigure(Vector2.Zero, isClosed: true);
        invalidFigure.Segments.Add(new RationalCubicBezierSegment(
            Vector2.UnitY,
            Vector2.One,
            Vector2.UnitX,
            0f,
        ...
        invalidFigure.Segments[0] = new RationalCubicBezierSegment(
            Vector2.UnitY,
            Vector2.One,
            Vector2.UnitX,
            1f,
        ...
        var rational = new PathGeometry();
        var rationalFigure = new PathFigure(Vector2.Zero, isClosed: true);
        rationalFigure.Segments.Add(new RationalCubicBezierSegment(
            Vector2.UnitY,
            Vector2.One,
            Vector2.UnitX,
            0.5f,
            1.5f));
        rational.Figures.Add(rationalFigure);
        Assert.Throws<NotSupportedException>(() =>
            PathOpGeometrySolver.Combine(rational, CreateOpenRectangle(), op: 2));
    }
