...
""";
        ...
        var binding = Assert.Single(DescendantValues(bindingDocument.Root).OfType<XamlBoundBinding>());
        Assert.Equal("Customer.Name", binding.Path);
        Assert.Equal(new[] { "Path", "Mode", "FallbackValue" },
            binding.Extension.Members.Select(member => member.Member.Symbol!.Name));
        Assert.All(binding.Extension.Members, member =>
        {
...
""";
    ...
    [Fact]
    public void WinUiSetterValueRetainsStructuredBindingDescriptor()
    {
        const string xaml = """
<Page xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
      xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
      x:Class="Demo.MainPage">
  <Setter Value="{Binding Path=PlaceholderForeground,
                          RelativeSource={RelativeSource TemplatedParent}}" />
</Page>
""";
        var compilation = CreateCompilation().AddSyntaxTrees(
            CSharpSyntaxTree.ParseText(
                "namespace Microsoft.UI.Xaml { public sealed class Setter { public object? Value { get; set; } } }"));
        var profile = new WinUiXamlProfile();
        var emitted = new CSharpXamlEmitter().Emit(
            Convert(xaml),
            new RoslynXamlTypeSystem(compilation, profile),
        ...
        var assignment = Assert.Single(
            ...
            candidate =>
                candidate.Left.ToString().EndsWith(".Value", StringComparison.Ordinal) &&
                candidate.Right is ObjectCreationExpressionSyntax creation &&
                creation.Type.ToString().EndsWith(
                    "Microsoft.UI.Xaml.Data.Binding",
                    StringComparison.Ordinal));
        var binding = Assert.IsType<ObjectCreationExpressionSyntax>(
            assignment.Right);
        Assert.Equal(
            new[] { "Path", "RelativeSource" },
            binding.Initializer!.Expressions
                .OfType<AssignmentExpressionSyntax>()
                .Select(item => item.Left.ToString()));
        Assert.DoesNotContain(
            compilation.AddSyntaxTrees(tree).GetDiagnostics(),
            diagnostic => diagnostic.Severity == DiagnosticSeverity.Error);
    }
...
""";
        ...
        var bound = new XamlSemanticBinder().Bind(
            ...
            new RoslynXamlTypeSystem(compilation, profile));
        Assert.False(bound.HasErrors, string.Join(Environment.NewLine, bound.Diagnostics));
        var ordinaryBinding = Assert.Single(
            DescendantValues(bound.Root).OfType<XamlBoundBinding>());
        Assert.Equal("Demo.Item", ordinaryBinding.SourceType!.MetadataName);
        Assert.Equal("Title", ordinaryBinding.PathSyntax!.Text);
        Assert.Equal(
            ordinaryBinding.Path,
            string.Concat(ordinaryBinding.PathSyntax.Tokens.Select(token => token.Text)));
        var accessor = Assert.Single(ordinaryBinding.Accessors);
        ...
        var setBinding = Assert.Single(
            ...
            invocation => string.Equals(
                ...
                StringComparison.Ordinal));
        Assert.Equal(7, setBinding.ArgumentList.Arguments.Count);
        Assert.Equal(
            "__templateLifetime",
            setBinding.ArgumentList.Arguments[6].Expression.ToString());
        ...
        Assert.Equal(
            2,
            registration.DescendantNodes()
                .OfType<ParenthesizedLambdaExpressionSyntax>()
                .Count(lambda => lambda.Modifiers.Any(SyntaxKind.StaticKeyword)));
        ...
        Assert.Contains(
            unsupported.Diagnostics,
            diagnostic => diagnostic.Id == "PGXAML3048");
        var rejectedAssignment = new CSharpXamlEmitter().Emit(
            Convert(xaml),
            new RoslynXamlTypeSystem(compilation, profile),
            new MissingDeferredBindingLifecycleProfile(),
            new XamlCompilerOptions());
        Assert.Contains(
            rejectedAssignment.Diagnostics,
            diagnostic => diagnostic.Id == "PGXAML3049");
    }

    [Fact]
    public void TypedOrdinaryBindingAccessorsAreDeduplicatedAndPreserveReadonlySymbols()
    {
        const string additions = """
namespace Microsoft.UI.Xaml {
  public class DataTemplate : FrameworkTemplate { }
...
namespace Demo {
  public sealed class Item {
    ...
    public static string Global { get; } = "static";
  }
...
<Page xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
  ...
  <DataTemplate x:DataType="local:Item">
    <StackPanel>
      <local:BindingTarget Value="{Binding Path=Child.Title}" />
      <local:BindingTarget Value="{Binding Path=Child.Title}" />
    </StackPanel>
  </DataTemplate>
</Page>
""";
        ...
        Assert.All(bindings, binding =>
        {
            ...
            Assert.Equal(new[] { "Child", "Title" },
                binding.Accessors.Select(accessor => accessor.Member.Name));
            Assert.All(binding.Accessors, accessor => Assert.False(accessor.CanWrite));
            Assert.All(binding.Accessors, accessor => Assert.IsAssignableFrom<IPropertySymbol>(accessor.Member));
        });
        ...
        Assert.Equal(
            ...
            registrations.Select(registration =>
                registration.ArgumentList.Arguments[0].ToString()));
        Assert.All(
            registrations,
            registration => Assert.Equal(
                "null",
                registration.ArgumentList.Arguments[2].Expression.ToString()));
        Assert.DoesNotContain(
            compilation.AddSyntaxTrees(tree).GetDiagnostics(),
            diagnostic => diagnostic.Severity == DiagnosticSeverity.Error);
...
""";
        var modelCompilation = CSharpCompilation.Create(
            "OrdinaryBindingMetadataModel",
            new[] { CSharpSyntaxTree.ParseText(metadataSource) },
            PlatformReferences(),
            new CSharpCompilationOptions(OutputKind.DynamicallyLinkedLibrary));
        using var modelImage = new System.IO.MemoryStream();
        var modelEmit = modelCompilation.Emit(modelImage);
        Assert.True(
            modelEmit.Success,
            string.Join(Environment.NewLine, modelEmit.Diagnostics));
        var metadataConsumer = CreateCompilation()
            .AddReferences(MetadataReference.CreateFromImage(modelImage.ToArray()))
            .AddSyntaxTrees(CSharpSyntaxTree.ParseText(
                "namespace Microsoft.UI.Xaml { public class DataTemplate : FrameworkTemplate { } }"));
        const string metadataXaml = """
<Page xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
      xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
      xmlns:model="using:MetadataModel"
      x:Class="Demo.MainPage">
  <DataTemplate x:DataType="model:Item">
    <TextBlock Text="{Binding Path=Child.Title}" />
  </DataTemplate>
</Page>
""";
        var metadataBound = new XamlSemanticBinder().Bind(
            Convert(metadataXaml),
            new RoslynXamlTypeSystem(metadataConsumer, profile));
        Assert.False(
            metadataBound.HasErrors,
            string.Join(Environment.NewLine, metadataBound.Diagnostics));
        var metadataBinding = Assert.Single(
            DescendantValues(metadataBound.Root).OfType<XamlBoundBinding>());
        Assert.Equal(2, metadataBinding.Accessors.Length);
        Assert.All(
            metadataBinding.Accessors,
            accessor =>
            {
                Assert.All(
                    accessor.Member.Locations,
                    location => Assert.True(location.IsInMetadata));
                Assert.All(
                    accessor.SourceType.Locations,
                    location => Assert.True(location.IsInMetadata));
            });
        var metadataEmission = new CSharpXamlEmitter().Emit(
            Convert(metadataXaml),
            new RoslynXamlTypeSystem(metadataConsumer, profile),
            profile,
            new XamlCompilerOptions());
        Assert.DoesNotContain(
            metadataEmission.Diagnostics,
            diagnostic => diagnostic.Severity == DiagnosticSeverity.Error);
        Assert.DoesNotContain(
            metadataConsumer.AddSyntaxTrees(
                Assert.Single(metadataEmission.Sources).GeneratedSyntaxTree!)
                .GetDiagnostics(),
            diagnostic => diagnostic.Severity == DiagnosticSeverity.Error);

        var unsupportedPath = new XamlSemanticBinder().Bind(
            Convert(xaml.Replace("Child.Title", "Child[0]", StringComparison.Ordinal)),
            typeSystem,
            new XamlSemanticBindingOptions { Strict = false });
        Assert.Contains(
            unsupportedPath.Diagnostics,
            diagnostic => diagnostic.Id == "PGXAML2132");
        var missingMember = new XamlSemanticBinder().Bind(
            Convert(xaml.Replace("Child.Title", "Missing", StringComparison.Ordinal)),
            typeSystem,
            new XamlSemanticBindingOptions { Strict = false });
        Assert.Contains(
            missingMember.Diagnostics,
            diagnostic => diagnostic.Id == "PGXAML2131");
        var staticMember = new XamlSemanticBinder().Bind(
            Convert(xaml.Replace("Child.Title", "Global", StringComparison.Ordinal)),
            typeSystem,
            new XamlSemanticBindingOptions { Strict = false });
        Assert.Contains(
...
""";
    ...
    [Fact]
    public void OrdinaryBindingBindsAndEmitsTypedIntegerAndStringIndexers()
    {
        const string additions = """
namespace Demo {
  public sealed class Item {
  ...
  public static class BindingSources {
    public static System.Collections.ObjectModel.ObservableCollection<Item> Items { get; } =
      new() { new Item() };
    public static System.Collections.Generic.IReadOnlyList<Item> ReadOnlyItems { get; } =
      System.Array.Empty<Item>();
    public static System.Collections.Generic.Dictionary<string, Item> Entries { get; } =
      new() { ["primary"] = new Item() };
  }
  public sealed class BindingTarget : Microsoft.UI.Xaml.FrameworkElement {
    public static readonly Microsoft.UI.Xaml.DependencyProperty ValueProperty = new();
    public string? Value { get; set; }
...
<Page xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
      ...
      x:Class="Demo.MainPage">
  <StackPanel>
    <local:BindingTarget Value="{Binding Path=[0].Title,
                                        Source={x:Static local:BindingSources.Items},
                                        Mode=TwoWay}" />
    <local:BindingTarget Value="{Binding Path=['primary'].Title,
                                        Source={x:Static local:BindingSources.Entries}}" />
  </StackPanel>
</Page>
""";
        var compilation = CreateCompilation().AddSyntaxTrees(
        ...
        Assert.Equal(
            ...
            bindings[0].Accessors.Select(accessor => accessor.Kind));
        Assert.Equal(0, bindings[0].Accessors[0].IntegerIndex);
        Assert.True(bindings[0].Accessors[0].CanWrite);
        Assert.Equal(
            new[]
            {
            ...
            bindings[1].Accessors.Select(accessor => accessor.Kind));
        Assert.Equal("primary", bindings[1].Accessors[0].StringIndex);
        ...
        Assert.Equal(
            ...
            tree.GetRoot().DescendantNodes()
                .OfType<InvocationExpressionSyntax>()
                .Count(invocation => invocation.Expression.ToString().EndsWith(
                    "BindingOperations.SetBindingWithPath",
                    StringComparison.Ordinal)));
        Assert.Contains(
            "BindingMemberAccessorRegistry.RegisterIndexer<global::System.Collections.ObjectModel.ObservableCollection<global::Demo.Item>, global::Demo.Item>",
            generated,
            StringComparison.Ordinal);
        Assert.Contains(
            "((global::System.Collections.Generic.IList<global::Demo.Item>)source)[0]",
            generated,
            StringComparison.Ordinal);
        Assert.Contains(
            "BindingMemberAccessorRegistry.RegisterIndexer<global::System.Collections.Generic.Dictionary<string, global::Demo.Item>, global::Demo.Item>",
            generated,
            StringComparison.Ordinal);
        Assert.Contains(
            "((global::System.Collections.Generic.IDictionary<string, global::Demo.Item>)source)[\"primary\"]",
            generated,
            StringComparison.Ordinal);
        Assert.Contains(
        ...
        var readOnly = new XamlSemanticBinder().Bind(
            Convert(xaml.Replace(
                "BindingSources.Items",
                "BindingSources.ReadOnlyItems",
                StringComparison.Ordinal)),
            new RoslynXamlTypeSystem(compilation, profile),
            new XamlSemanticBindingOptions { Strict = false });
        Assert.Contains(
...
""";
    ...
    [Fact]
    public void ConstructionTransformsComposeInOrderAndEmitRewrittenIr()
    {
        const string xaml = """
<Page xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
      xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
""";
        ...
        var host = RoslynXamlExtensionHost.Create(
            new IRoslynXamlExtension[]
            {
                new TestConstructionTransform(
                    ...
                    from: "High",
                    to: "Final",
                    diagnosticId: "TESTT0001",
                    expectedResourceUri: "Views/Transform.xaml"),
                new TestConstructionTransform(
                    "test.transform.high",
                    priority: 20,
                    from: "Hello",
        ...
        var result = new CSharpXamlEmitter(host).Emit(
            ConvertAt("Transform.xaml", xaml),
            new RoslynXamlTypeSystem(compilation, profile),
            profile,
            new XamlCompilerOptions { ResourceUri = "Views/Transform.xaml" },
            cancellationSource.Token);

        var diagnostic = Assert.Single(
            result.Diagnostics,
            item => item.Id == "TESTT0001");
        Assert.Equal("Transform.xaml", diagnostic.Location.GetLineSpan().Path);
        var tree = Assert.Single(result.Sources).GeneratedSyntaxTree!;
        Assert.Contains(
            tree.GetRoot().DescendantTokens(),
            token => token.IsKind(SyntaxKind.StringLiteralToken) &&
    ...
    [Fact]
    public void ConstructionTransformFailuresAreIsolatedAndCancellationPropagates()
    {
        const string xaml = """
<Page xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
      xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
""";
        ...
        var failingHost = RoslynXamlExtensionHost.Create(
            new IRoslynXamlExtension[]
            {
                ...
                new TestConstructionTransform(
                    ...
                    returnInvalidRoot: true),
                new TestConstructionTransform(
                    "test.transform.null-result",
                    priority: 50,
                    from: "Hello",
                    to: "unused",
                    returnNullResult: true),
                new TestConstructionTransform(
                    "test.transform.null-root",
                    priority: 40,
                    from: "Hello",
                    to: "unused",
                    returnNullRoot: true),
                new TestConstructionTransform(
                    "test.transform.invalid-span",
                    priority: 30,
                    from: "Hello",
                    to: "unused",
                    returnInvalidOperationSpan: true),
                new TestConstructionTransform(
                    "test.transform.invalid-shape",
                    priority: 20,
                    from: "Hello",
                    to: "unused",
                    returnDefaultValues: true),
                new TestConstructionTransform(
                    "test.transform.continues",
                    priority: 10,
                    from: "Hello",
                    to: "continued",
                    diagnosticId: "TESTT1000")
            });
        ...
        Assert.Single(failure.Diagnostics, diagnostic => diagnostic.Id == "PGXAML3052");
        Assert.Equal(
            5,
            failure.Diagnostics.Count(diagnostic => diagnostic.Id == "PGXAML3053"));
        Assert.Single(failure.Diagnostics, diagnostic => diagnostic.Id == "TESTT1000");
        Assert.Empty(failure.Sources);

        var cancellationHost = RoslynXamlExtensionHost.Create(
            new IRoslynXamlExtension[]
            {
                new TestConstructionTransform(
                    "test.transform.cancellation",
                    priority: 10,
                    from: "Hello",
                    to: "unused",
                    cancelOnInvoke: true)
    ...
    [Fact]
    public void ConstructionTransformAuditIsBoundedAndHandlesDeepIrIteratively()
    {
        const int depth = 200;
        var xaml =
            """
<Page xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
      ...
      x:Class="Demo.MainPage">
""" +
            string.Concat(Enumerable.Repeat("<StackPanel>", depth)) +
            """<TextBlock Text="Hello" />""" +
            string.Concat(Enumerable.Repeat("</StackPanel>", depth)) +
            "</Page>";
        var compilation = CreateCompilation();
        var profile = new WinUiXamlProfile();
        var transform = new TestConstructionTransform(
            "test.transform.deep",
            priority: 10,
            from: "unmatched",
            to: "unused");
        var defaultHost = RoslynXamlExtensionHost.Create(
            new IRoslynXamlExtension[] { transform });

        var transformed = new CSharpXamlEmitter(defaultHost).Emit(
            Convert(xaml),
            new RoslynXamlTypeSystem(compilation, profile),
            profile,
            new XamlCompilerOptions());
        ...
        Assert.DoesNotContain(
            ...
            diagnostic => diagnostic.Severity == DiagnosticSeverity.Error);
        Assert.DoesNotContain(
            compilation.AddSyntaxTrees(Assert.Single(transformed.Sources).GeneratedSyntaxTree!)
                .GetDiagnostics(),
            diagnostic => diagnostic.Severity == DiagnosticSeverity.Error);

        var boundedHost = RoslynXamlExtensionHost.Create(
            new IRoslynXamlExtension[] { transform },
            new RoslynXamlExtensionHostOptions { MaximumTransformedIrNodes = 10 });
        var bounded = new CSharpXamlEmitter(boundedHost).Emit(
            Convert(xaml),
            new RoslynXamlTypeSystem(compilation, profile),
            profile,
        ...
        Assert.Contains(bounded.Diagnostics, diagnostic => diagnostic.Id == "PGXAML3053");
...
""";
        ...
        IRoslynXamlConstructionProgramTransformExtension
    {
        ...
        private readonly bool _returnNullResult;
        private readonly bool _returnNullRoot;
        private readonly bool _returnInvalidOperationSpan;
        private readonly bool _returnDefaultValues;
        private readonly bool _cancelOnInvoke;
        private readonly CancellationToken? _expectedCancellationToken;
        private readonly string? _expectedResourceUri;

        public TestConstructionTransform(
            string id,
            int priority,
            string from,
            string to,
            ...
            bool returnNullResult = false,
            bool returnNullRoot = false,
            bool returnInvalidOperationSpan = false,
            bool returnDefaultValues = false,
            bool cancelOnInvoke = false,
            CancellationToken? expectedCancellationToken = null,
            string? expectedResourceUri = null)
        {
            ...
            _returnNullResult = returnNullResult;
            _returnNullRoot = returnNullRoot;
            _returnInvalidOperationSpan = returnInvalidOperationSpan;
            ...
            RoslynXamlConstructionTransformContext context)
        {
            ...
            if (_expectedCancellationToken is { } expected &&
                context.CancellationToken != expected)
                throw new InvalidOperationException("The transform did not receive the emitter token.");
            if (!string.Equals(context.FrameworkId, "WinUI", StringComparison.Ordinal) ||
                !context.Strict ||
                !string.Equals(
                    context.ResourceUri,
                    _expectedResourceUri,
                    StringComparison.Ordinal))
                throw new InvalidOperationException("The transform did not receive compiler identity/options.");
            if (context.Program.ResourceGraph != null &&
                !ReferenceEquals(
                    context.Program.BoundDocument,
                    context.Program.ResourceGraph.Document))
                throw new InvalidOperationException("The transform lost bound/resource ownership.");
            if (_throwOnInvoke)
                throw new InvalidOperationException("Expected transform failure.");
            if (_cancelOnInvoke)
                throw new OperationCanceledException();
            if (_returnNullResult)
                return null!;

            var root = context.Program.Root == null
                ? null
                : RewriteObject(context.Program.Root);
            if (_returnNullRoot)
                root = null;
            if (_returnInvalidRoot && root != null)
            {
                root = new XamlIrObject(
                    root.Kind,
                    root.InitializationMode,
                    root.Type,
                    root.Operations,
                    root.SourceSpan,
                    root.StableId + 1);
            }
            if ((_returnInvalidOperationSpan || _returnDefaultValues) &&
                root != null &&
                root.Operations.Length != 0)
            {
                var operation = root.Operations[0];
                var replacement = new XamlIrOperation(
                    operation.Kind,
                    operation.Member,
                    _returnDefaultValues
                        ? default
                        : operation.Values,
                    _returnInvalidOperationSpan
                        ? new TextSpan(
                            context.Program.BoundDocument.Infoset.SourceText.Length + 1,
                            0)
                ...
                root = new XamlIrObject(
                    ...
                    root.InitializationMode,
                    root.Type,
                    root.Operations.SetItem(0, replacement),
                    root.SourceSpan,
                    root.StableId);
            }
            var issues = _diagnosticId == null || root == null
                ...
                : new[]
                {
                    new RoslynXamlValidationIssue(
                        _diagnosticId,
                        DiagnosticSeverity.Warning,
                        "Construction transform result.",
                        root.SourceSpan,
                        "EXT-004")
                };
            return new RoslynXamlConstructionTransformResult(root, issues);
        }

        private XamlIrObject RewriteObject(XamlIrObject value)
        {
            var operations = value.Operations.Select(operation =>
                new XamlIrOperation(
                    operation.Kind,
                    operation.Member,
                    operation.Values.Select(RewriteValue).ToImmutableArray(),
                    operation.SourceSpan,
                    operation.StableId)).ToImmutableArray();
        ...
        private XamlIrValue RewriteValue(XamlIrValue value)
        {
            if (value is XamlIrText text &&
                string.Equals(text.Text, _from, StringComparison.Ordinal))
            {
                return new XamlIrText(
                    _to,
                    text.SourceSpan,
                    text.StableId,
                    text.TextSyntax);
            }
            return value is XamlIrObject objectValue
                ? RewriteObject(objectValue)
                : value;
        }
    }
    ...
    private sealed class InvalidConstructionTransformExtension : IRoslynXamlExtension
    {
        public string Id => "test.invalid-transform";
        public int ContractVersion => RoslynXamlExtensionHost.CurrentContractVersion;
        public int Version => 1;
        public int Priority => 0;
        public RoslynXamlExtensionCapabilities Capabilities =>
...
""";
    ...
    [Fact]
    public async Task ProjectPreviewCoordinatorCommitsOnlyHostAcceptedCurrentUpdates()
    {
...
""";
        ...
        var initial = await coordinator.PrepareAsync(
            fixture.Project,
            fixture.TargetXamlDocumentId);
        Assert.True(initial.IsInitial);
        Assert.True(initial.CanCommit);
        Assert.True(initial.RequiresRuntimePublication);
        Assert.Equal(
        ...
        var syntaxOnly = await coordinator.PrepareAsync(
            fixture.Project,
            fixture.TargetXamlDocumentId,
            SourceText.From(syntaxOnlyChange));
        Assert.Equal(
            RoslynXamlReloadAction.None,
            syntaxOnly.Delta!.Action);
        Assert.False(
            syntaxOnly.RequiresRuntimePublication);
        Assert.Equal(
            RoslynXamlProjectCommitResult.Accepted,
            coordinator.TryCommit(syntaxOnly));
        ...
        var first = await coordinator.PrepareAsync(
            firstProject,
            fixture.TargetXamlDocumentId);
        var stale = await coordinator.PrepareAsync(
            secondProject,
            fixture.TargetXamlDocumentId);
        Assert.Equal(
            RoslynXamlReloadAction.ReplaceTarget,
        ...
        Assert.Equal(0, stalePublicationCount);
        Assert.Same(first.Current, coordinator.LastAccepted);
        Assert.Equal(3, coordinator.Generation);

        var failed = await coordinator.PrepareAsync(
            WithAdditionalDocumentText(
                firstProject,
                fixture.TargetXamlDocumentId,
                invalid),
            fixture.TargetXamlDocumentId);
        Assert.False(failed.CanCommit);
        Assert.NotNull(failed.FailureMessage);
        Assert.Equal(
            RoslynXamlReloadAction.RetainLastGood,
            failed.Delta!.Action);
        Assert.Equal(
            ...
            coordinator.TryCommit(failed));
        Assert.Same(first.Current, coordinator.LastAccepted);
        Assert.Equal(3, coordinator.Generation);

        var recovered = await coordinator.PrepareAsync(
            secondProject,
            fixture.TargetXamlDocumentId);
        Assert.True(recovered.CanCommit);
        Assert.Equal(
            RoslynXamlProjectCommitResult.Accepted,
            coordinator.TryCommit(recovered));
        Assert.Same(
            recovered.Current,
            coordinator.LastAccepted);
