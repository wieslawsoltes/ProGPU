...
public sealed class DrawingApiQualityTests
{
    ...
    public void ImageAttributesSnapshotColorMatrixAndRejectUseAfterDispose()
    {
        ...
        attributes.Dispose();
        Assert.Throws<ArgumentException>(() => attributes.ClearColorMatrix());
    }
    ...
    public void ImageAttributesApplyColorKeyGammaAndThresholdInDocumentedOrder()
    {
        ...
        Assert.Equal(Color.Blue.ToArgb(), adjusted.GetPixel(1, 0).ToArgb());
    ...
    public void MatrixSpanVectorInverseAndOwnershipContractsAreFunctional()
    {
        ...
        matrix.Dispose();
        Assert.Throws<ArgumentException>(() => _ = matrix.Elements);
    }

    [Fact]
    public void MatrixMatchesDesktopValidationIdentityHashAndOverflowContracts()
    {
        Assert.Throws<ArgumentException>(() => new Matrix(default(RectangleF), Array.Empty<PointF>()));
        Assert.Throws<System.Runtime.InteropServices.ExternalException>(
            () => new Matrix(new RectangleF(0f, 0f, 0f, 1f), new PointF[3]));

        using var approximateIdentity = new Matrix(1.0001f, 0.0001f, -0.0001f, 0.9999f, 0f, 0f);
        Assert.True(approximateIdentity.IsIdentity);

        using var nonFinite = new Matrix(float.NaN, 0f, 0f, 1f, 0f, 0f);
        Assert.False(nonFinite.IsInvertible);
        Assert.Throws<ArgumentException>(nonFinite.Invert);

        using var value1 = new Matrix(1f, 2f, 3f, 4f, 5f, 6f);
        using var value2 = new Matrix(1f, 2f, 3f, 4f, 5f, 6f);
        Assert.True(value1.Equals(value2));
        Assert.NotEqual(value1.GetHashCode(), value2.GetHashCode());
        Assert.Throws<InvalidOperationException>(() => value1.Multiply(value1));
        Assert.Throws<ArgumentException>(() => value1.TransformPoints(Array.Empty<PointF>()));

        using var overflow = new Matrix(1f, 2f, 3f, 4f, 5f, 6f);
        overflow.Translate(float.MaxValue, float.MaxValue, MatrixOrder.Prepend);
        Assert.Equal(float.MaxValue, overflow.OffsetX);
        Assert.Equal(float.MaxValue, overflow.OffsetY);

        value1.Dispose();
        Assert.Throws<ArgumentException>(() => value1.Reset());
    }
