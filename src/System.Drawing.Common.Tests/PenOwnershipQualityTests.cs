...
public sealed class PenOwnershipQualityTests
{
    ...
    public void BrushConstructorSnapshotsInputAndGetterReturnsIndependentClones()
    {
        ...
        Assert.Equal(Color.Red.ToArgb(), second.Color.ToArgb());
    ...
    [Fact]
    public void KnownColorBrushesAreMutableAndKnownColorPensRemainImmutable()
    {
        var brush = Assert.IsType<SolidBrush>(Brushes.Red);
        Pen pen = Pens.Red;

        Color original = brush.Color;
        brush.Color = Color.Blue;
        Assert.Equal(Color.Blue, brush.Color);
        brush.Color = original;
