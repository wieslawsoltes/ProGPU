...
public sealed class PrinterSettingsCollectionQualityTests
{
    ...
    [Fact]
    public void PrinterSettingsPreservesManagedValidationAndDiagnosticContract()
    {
        var settings = new PrinterSettings();

        Assert.Throws<ArgumentException>(() => settings.Copies = -1);
        Assert.Throws<ArgumentException>(() => settings.FromPage = -1);
        Assert.Throws<ArgumentException>(() => settings.MinimumPage = -1);
        Assert.Throws<ArgumentException>(() => settings.MaximumPage = -1);
        Assert.Throws<ArgumentException>(() => settings.ToPage = -1);
        Assert.Throws<InvalidEnumArgumentException>(() => settings.Duplex = (Duplex)(-2));
        Assert.Throws<InvalidEnumArgumentException>(() => settings.PrintRange = (PrintRange)(-1));
        Assert.Throws<ArgumentNullException>(() => settings.PrintFileName = null!);
        Assert.Throws<ArgumentNullException>(() => settings.PrintFileName = string.Empty);

        Assert.True(settings.IsDefaultPrinter);
        Assert.Equal(
            "[PrinterSettings  Copies=1 Collate=False Duplex=Default FromPage=0 LandscapeAngle=0 MaximumCopies=1 OutputPort= ToPage=0]",
            settings.ToString());
        Assert.Throws<NullReferenceException>(() => settings.CreateMeasurementGraphics(null!));
        Assert.Throws<ArgumentException>(() => settings.SetHdevmode(IntPtr.Zero));
    }
