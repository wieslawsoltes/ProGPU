...
internal static class KnownColorResources
{
    ...
    private static readonly Brush?[] s_systemBrushes = new Brush[CacheCapacity];
    ...
    public static Brush GetBrush(KnownColor knownColor)
    {
        ...
        var created = new SolidBrush(Color.FromKnownColor(knownColor), immutable: false);
        return Interlocked.CompareExchange(ref s_brushes[index], created, null) ?? created;
    }

    public static Brush GetSystemBrush(KnownColor knownColor)
    {
        int index = GetIndex(knownColor);
        Brush? current = Volatile.Read(ref s_systemBrushes[index]);
        if (current is not null)
        {
            return current;
        }
        ...
        return Interlocked.CompareExchange(ref s_systemBrushes[index], created, null) ?? created;
    }
