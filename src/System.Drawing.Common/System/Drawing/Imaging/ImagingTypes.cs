...
public sealed class ImageAttributes : IDisposable, ICloneable
{
        ...
        ColorAdjustType type)
    {
        ...
        ValidateColorMatrixFlag(mode);
        ValidateColorAdjustType(type);
        if (mode == ColorMatrixFlag.AltGrays && grayMatrix is null)
        {
            throw new ArgumentException("The alternate-gray matrix cannot be null.");
        }
    ...
    public void SetRemapTable(ColorAdjustType type, ReadOnlySpan<(Color OldColor, Color NewColor)> map)
    {
        ...
        if (map.IsEmpty)
        {
            throw new ArgumentException("A remap table cannot be empty.");
        }
    ...
    public void SetOutputChannel(ColorChannelFlag flags, ColorAdjustType type)
    {
        ...
        {
            throw new ArgumentException("Invalid output color channel.");
        }
    ...
    public void SetOutputChannelColorProfile(string colorProfileFilename, ColorAdjustType type)
    {
        ...
        ValidateColorAdjustType(type);
        if (colorProfileFilename.Length >= short.MaxValue)
        {
            throw new PathTooLongException("The color profile path is too long.");
        }

        if (!File.Exists(colorProfileFilename))
        {
            throw new ExternalException("The color profile could not be opened.");
        }
    ...
    public void SetWrapMode(Drawing2D.WrapMode mode, Color color, bool clamp)
    {
        ...
        {
            throw new ArgumentException("Invalid wrap mode.");
        }
    ...
    private void ThrowIfDisposed()
    {
        if (_isDisposed)
        {
            throw new ArgumentException("ImageAttributes has been disposed.");
        }
    }
    ...
    private static void ValidateColorAdjustType(ColorAdjustType type)
    {
        ...
        {
            throw new ArgumentException("Invalid color-adjust type.");
        }
    }
    ...
    private static void ValidateColorMatrixFlag(ColorMatrixFlag flag)
    {
        ...
        {
            throw new ArgumentException("Invalid color-matrix flag.");
        }
    }
    ...
    private static byte ApplyGamma(byte value, float gamma) =>
        (byte)ToByte(MathF.Pow(value / 255f, gamma));
