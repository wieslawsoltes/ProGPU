...
public static unsafe class NativeDocumentFlow
{
    ...
    public static NativeDocumentFlowResult ArrangeWithContentMeasurement(
        ...
        Span<NativeDocumentLinePosition> positions, out double contentWidth, NativeMilBackend backend = NativeMilBackend.WgpuNative)
    {
        Validate(blocks.Length, width, boxes.Length, backend);
        ValidateRows(rows.Length, columnWidths.Length, cells.Length);
        if (lines.Length > MaximumItems) throw new ArgumentOutOfRangeException(nameof(lines));
        ...
        fixed (NativeDocumentLinePosition* placed = positions)
            status = backend == NativeMilBackend.Dawn
                ? NativeDawnDocumentFlowMethods.ArrangeWithContentMeasurement(input, (uint)blocks.Length, width,
                    metrics, (uint)lines.Length, measured, (uint)objects.Length,
                    rowInput, (uint)rows.Length, columns, (uint)columnWidths.Length, cellInput, (uint)cells.Length,
                    paragraphInput, (uint)paragraphs.Length, local, (uint)localPositions.Length,
                    output, (uint)boxes.Length, placed, (uint)positions.Length, &result, &measuredWidth)
                : NativeDocumentFlowMethods.ArrangeWithContentMeasurement(input, (uint)blocks.Length, width,
                    metrics, (uint)lines.Length, measured, (uint)objects.Length,
                    rowInput, (uint)rows.Length, columns, (uint)columnWidths.Length, cellInput, (uint)cells.Length,
                    paragraphInput, (uint)paragraphs.Length, local, (uint)localPositions.Length,
                    output, (uint)boxes.Length, placed, (uint)positions.Length, &result, &measuredWidth);
        if (status != NativeRendererStatus.Success)
            throw new NativeRendererException(status, "Native document content measurement failed.");
...
{
    [LibraryImport(NativeMethods.LibraryName, EntryPoint = "progpu_native_document_arrange_with_content_measurement")]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    internal static partial NativeRendererStatus ArrangeWithContentMeasurement(NativeDocumentBlock* blocks, uint count, double width,
        ...
        NativeDocumentBox* boxes, uint capacity, NativeDocumentLinePosition* positions, uint positionCapacity, NativeDocumentFlowResult* result, double* contentWidth);
...
{
    [LibraryImport(NativeDawnMethods.LibraryName, EntryPoint = "progpu_native_document_arrange_with_content_measurement")]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    internal static partial NativeRendererStatus ArrangeWithContentMeasurement(NativeDocumentBlock* blocks, uint count, double width,
        NativeDocumentLine* lines, uint lineCount, NativeDocumentObject* objects, uint objectCount,
        NativeDocumentRow* rows, uint rowCount, double* columns, uint columnCount, NativeDocumentCell* cells, uint cellCount,
