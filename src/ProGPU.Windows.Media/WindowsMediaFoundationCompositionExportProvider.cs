    ...
    IMediaCompositionExportCapabilityProvider
{
        ...
        MediaEffectRegistry effects)
    {
        ...
        return new MediaCompositionExportCapabilities(
            ...
            Limitation:
                ...
                "definitions are combined and applied in-place to native " +
                "PCM16 buffers. Custom overlay compositors, arbitrary " +
                "effects, and audio mixing are not yet accepted. Ordered " +
                "video overlays use " +
                "retained Media Foundation readers and source-over WebGPU " +
                "composition; audible overlay audio remains unsupported " +
                "when AAC output is requested. The native PCM16 channel " +
                "level lane supports 0–2× with saturation.");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!TryCaptureAudioLevels(
                request,
                _effects,
                out MediaAudioStereoLevels[] audioLevels))
        {
            return ValueTask.FromResult(
                MediaCompositionExportFailure.InvalidProfile);
        }
    ...
    private static MediaCompositionExportFailure
        RenderCore(
        MediaCompositionExportRequest request,
        IReadOnlyList<MediaAudioStereoLevels>
            audioLevels,
    ...
    private static void RenderClip(
        ...
        long timelineOffset,
        in WindowsGpuVideoEffectPlan effectPlan,
        in MediaAudioStereoLevels audioLevels,
    ...
    private static bool TryCaptureAudioLevels(
        MediaCompositionExportRequest request,
        MediaEffectRegistry effects,
