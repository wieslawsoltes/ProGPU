...
internal static class MacInstrumentsCapture
{
    ...
    public static int Run(string[] args)
    {
        ...
        foreach (string templateKey in options.Templates)
        {
            ...
            recordArguments.Add("--");
            recordArguments.Add(options.Executable);
            recordArguments.AddRange(options.ExecutableArguments);
        ...
        out string? error)
    {
        ...
        if (launchIndex < 0 || launchIndex + 1 >= args.Length)
        {
            options = default!;
            error = "The Instruments command requires --launch <executable>.";
            return false;
        }

        for (int index = 1; index < launchIndex; index++)
        {
            string argument = args[index];
            if (argument == "--output" && index + 1 < launchIndex)
            {
                output = args[++index];
            }
            ...
            else if (argument == "--templates" && index + 1 < launchIndex)
            {
        ...
        string executableArgument = args[launchIndex + 1];
        string executable =
            Path.IsPathFullyQualified(executableArgument) ||
            executableArgument.Contains(Path.DirectorySeparatorChar) ||
            executableArgument.Contains(Path.AltDirectorySeparatorChar)
        ...
        options = new CaptureOptions(
            ...
            allocationDetails,
            executable,
            args.Skip(launchIndex + 2).ToArray());
        error = null;
        return true;
    }
