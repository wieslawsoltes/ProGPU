...
Image metadata uses the complete `SKImageInfo` value contract. Platform RGBA shifts, byte/bit depth, checked 32-bit and wide 64-bit row/image sizes, emptiness, opacity, rectangles, equality, and immutable-style `With*` copies are derived without touching pixel storage or initializing WebGPU. Every query is `O(1)` time and space; overflow remains explicit on the native 32-bit size properties instead of wrapping into an undersized allocation.
