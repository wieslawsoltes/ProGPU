...
scalars; the explicit SOURCE_BIDI policy resolves bidi on original source scalars
while retaining substituted glyphs for WPF source parity. Contextual state
crosses style boundaries and uses the
...
Retained bidi levels and interaction must reuse the same selected source/glyph
bidi policy as shaping; never independently re-resolve bidi with a different
policy. Preserve original source indices and the existing metadata
scratch/crossing budget. Validate actual decimal values zero through nine, not
only a continuous Nd range, and unknown
